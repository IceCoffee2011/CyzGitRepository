...
#define SUB_GR_QUERY_TRANSRECORD  6										//转账记录
#define SUB_GR_QUERY_NICKNAME_BY_GAMEID	7								// 根据GameID查询昵称
...
#define SUB_GR_TRANS_RECORD				104                             //转账记录
#define SUB_GR_USER_TRANSFER_RECEIPT	105								// 转账回执	
#define SUB_GR_VERIFY_INSURE_PASSWORD	106								// 验证银行密码
#define SUB_GR_MODIFY_INSURE_PASS		107								// 更改银行密码

//////////////////////////////////////////////////////////////////////////////////
//转账记录
struct CMD_GR_RealTransRecord
{
    DWORD                               dwGameID;
...
// 验证用户密码信息
struct CMD_GR_VerifyUserInsurePassword
{
...
#define MDM_GR_USERCUSTOM						8									//自建房间命令
#define SUB_GR_USER_JOIN_TABLE					401									//用户加入自建桌子

// struct CMD_GR_UserCustomTable
// {
// 	WORD	wKindID;							// 游戏ID
// 	WORD	wServerID;							// 房间编号
// 	WORD	wTableCardCount;					// 消耗多少房卡
// 	BYTE	cbCustomRule[LEN_CUSTOMRULE];		// 自定规则
// };

// 加入自定义桌子
struct CMD_MB_JoinCustomTable
{
	DWORD	dwUserID;							// 用户ID
	WORD	wKindID;								
	WORD	wServerID;
	TCHAR	szSecret[LEN_CUSTOMTABLEPASSWORD];	// 房间密码
};
...
#define SUB_GF_APPLY_FOR_DISMISS	4									// 申请解散
#define SUB_GF_CHOICE_FOR_DISMISS	5									// 玩家对解散的选择
...
#define SUB_GF_CUSTOMTABLE_INFO		103									//自建桌子创建者
#define SUB_GF_S_CONLUDETABLE		104									// 桌子结算结果通知
...
//桌子创建者
struct CMD_GF_CustomTableInfo
{
	DWORD							dwUserID;							// 自建桌子的创建者
	bool							bTableStarted;						// 桌子是否开始
...
// 用户申请解散桌子(客户端→服务端)
struct CMD_GF_C_UserApplyForDismiss
{
	DWORD							dwUserID;							// 发起解散申请的玩家
};

// 用户申请解散桌子(服务端→客户端)
struct CMD_GF_S_UserApplyForDismiss
{
	DWORD							dwUserID;							// 发起解散申请的玩家
};

// 用户投票结果(客户端→服务端)
struct CMD_GF_C_UserChoiceForDismiss
{
	DWORD							dwUserID;							// 玩家选择
	bool							bDismiss;							// 是否解散
};
...
// 用户投票结果(服务端→客户端)
struct CMD_GF_S_UserChoiceForDismiss
{
	DWORD							dwUserID;							// 玩家选择
	bool							bDismiss;							// 是否解散
};
...
// 解散结果相关信息
// 解散自建桌子
#define CONCLUDE_TYPE_NORMAL				1				// 1:打满局数,正常结算;
#define CONCLUDE_TYPE_CREATER_DISMISS		2				// 2:未开始游戏,创建者解散;
#define CONCLUDE_TYPE_TIMEOUT_NOSTART		3				// 3:未开始游戏,到时间自动解散; 
#define CONCLUDE_TYPE_PLAYER_DISMISS		4				// 4:游戏中玩家申请解散
#define CONCLUDE_TYPE_TIMEOUT_START			5				// 5:开始游戏后,超时解散
struct CMD_GF_S_ConludeTable
{
	WORD wConcludeType;													// 房间结算类型
	TCHAR szMessage[LEN_USER_CHAT];										// 提示信息
	DWORD dwTableCreater;												// 桌子创建者
	DWORD dwDiamondCount;												// 创建者剩余钻石数量
};
...
#define DTP_GR_TABLE_INFO			30									//桌子附加信息		

//会员信息
struct DTP_GR_TableInfo
{
	TCHAR							szTableName[MAX_TABLE][LEN_TABLENAME];		// 桌子昵称
	unsigned char					cTableNormalPicID[MAX_TABLE];				// 房间普通桌子显示状态
	unsigned char					cTablePlayingPicID[MAX_TABLE];				// 房间游戏中桌子状态
};
