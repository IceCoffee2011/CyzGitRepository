...
#define SR_ALLOW_TRUSTEE_WRITESCORE	0x00000080							//托管代打允许结算离开
...
// 银行规则补充
#define SR_FORFEND_TRANSFER_IN_ROOM		0x01000000						//禁止房间转账
#define SR_FORFEND_TRANSFER_IN_GAME		0x02000000						//禁止游戏转账

#define SR_FORFEND_CALCULATE_BALANCE_SCORE		0x10000000				// 禁止结算运气值
#define SR_FORFEND_GET_BALANCE_SCORE	0x20000000						// 禁止获取运气值
...
public:
	...
	// 断线代打允许结算离开
	static bool IsAllowTrusteeWriteScore(DWORD dwServerRule) { return (dwServerRule&SR_ALLOW_TRUSTEE_WRITESCORE) != 0; }
...
//	static bool IsForfendTakeInGame(DWORD dwServerRule) { return (dwServerRule&SR_FORFEND_TAKE_IN_GAME)!=0; }
	static bool IsForfendTakeInGame(DWORD dwServerRule) { return true; }
	//禁止存钱
...
//	static bool IsForfendSaveInGame(DWORD dwServerRule) { return (dwServerRule&SR_FORFEND_SAVE_IN_GAME)!=0; }
	static bool IsForfendSaveInGame(DWORD dwServerRule) { return true; }
	//禁止房间转账
	static bool IsForfendTransferInRoom(DWORD dwServerRule) { return (dwServerRule&SR_FORFEND_TRANSFER_IN_ROOM)!=0; }
	//禁止游戏转账(任何时候都禁止,配置无关)
//	static bool IsForfendTransferInGame(DWORD dwServerRule) { return (dwServerRule&SR_FORFEND_TRANSFER_IN_GAME)!=0; }
	static bool IsForfendTransferInGame(DWORD dwServerRule) { return true; }
...
public:
	...
	// 是否禁止计算运气值(全部禁用,统一返回true)
//	static bool IsForfendCalculateBalanceScore(DWORD dwServerRule) { return (dwServerRule&SR_FORFEND_CALCULATE_BALANCE_SCORE)!=0; }
	static bool IsForfendCalculateBalanceScore(DWORD dwServerRule) { return true; }
	// 是否禁止获取运气值
//	static bool IsForfendGetBalanceScore(DWORD dwServerRule) { return (dwServerRule&SR_FORFEND_GET_BALANCE_SCORE)!=0; }
	static bool IsForfendGetBalanceScore(DWORD dwServerRule) { return true; }
...
public:
	...
	// 代打结算离开
	static VOID SetAllowTrusteeWriteScore(DWORD & dwServerRule, bool bEnable) { (bEnable == true) ? dwServerRule |= SR_ALLOW_TRUSTEE_WRITESCORE : dwServerRule &= ~SR_ALLOW_TRUSTEE_WRITESCORE; }
...
public:
	...
	//禁止房间转账
	static VOID SetForfendTransferInRoom(DWORD & dwServerRule, bool bEnable) { (bEnable==true)?dwServerRule|=SR_FORFEND_TRANSFER_IN_ROOM:dwServerRule&=~SR_FORFEND_TRANSFER_IN_ROOM; }
	//禁止游戏转账
	static VOID SetForfendTransferInGame(DWORD & dwServerRule, bool bEnable) { (bEnable==true)?dwServerRule|=SR_FORFEND_TRANSFER_IN_GAME:dwServerRule&=~SR_FORFEND_TRANSFER_IN_GAME; }
...
public:
	...
	// 禁止计算运气值
	static VOID SetForfendCalculateBalanceScore(DWORD & dwServerRule, bool bEnable) { (bEnable==true)?dwServerRule|=SR_FORFEND_CALCULATE_BALANCE_SCORE:dwServerRule&=~SR_FORFEND_CALCULATE_BALANCE_SCORE; }
	// 禁止获取运气值
	static VOID SetForfendGetBalanceScore(DWORD & dwServerRule, bool bEnable) { (bEnable==true)?dwServerRule|=SR_FORFEND_GET_BALANCE_SCORE:dwServerRule&=~SR_FORFEND_GET_BALANCE_SCORE; }
};
