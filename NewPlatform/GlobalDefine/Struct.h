...
{
	WORD		wKindID;												// 自建桌子的游戏ID
	WORD		szPropertyConsumeCount[LEN_CUSTOMTABLEPROPERTYCONFIG];	// 自建桌子的游戏道具消耗配置
	WORD		szRoundCount[LEN_CUSTOMTABLEPROPERTYCONFIG];			// 自建桌子的游戏局数配置
};
...
struct tagGameTableOption
{
	...
	//房间配置
	SCORE							lRestrictScore;						//最大输赢限制
	SCORE							lMinTableScore;						//最低积分
	SCORE							lMaxTableScore;						//最低积分

	//会员限制
	BYTE							cbMinTableEnterMember;				//最低会员
	BYTE							cbMaxTableEnterMember;				//最高会员
	
	// 自定义规则
	BYTE							cbCustomRule[1024];					//自定规则

	// 桌子图片相关配置
	int								nNormalPicID;						// 普通状态图片
	int								nPlayingPicID;						// 游戏状态图片

	// 一局游戏最长时间
	int								nMaxGameTime;						// 最长游戏时间

	// 自建桌子相关配置
	DWORD							dwCreatUserID;						// 桌子创建者
	TCHAR							szSecret[LEN_CUSTOMTABLEPASSWORD];	// 自建桌子密码
	DWORD							dwRoundCount;						// 创建的局数
	DWORD							dwPlayRoundCount;					// 已经游戏的局数(从数据库中载入的数据,每局不维护)
...
{
	vector<tagGameTableOption>		TableOption;						// 桌子配置
};
...
struct tagDataBaseParameter
{
	...
	TCHAR							szDataBaseAddr[512];					//数据库地址
	TCHAR							szDataBaseUser[512];					//数据库用户
	TCHAR							szDataBasePass[512];					//数据库密码
	TCHAR							szDataBaseName[512];					//数据库名字
};
