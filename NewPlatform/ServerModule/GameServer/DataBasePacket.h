...
#define DBR_GR_LOAD_BALANCE_SCORE_CURVE 206									//平衡分曲线
#define DBR_GR_RESET_GAME_SCORE_LOCKER	207									//重置游戏锁
#define DBR_GR_SQUARE_REWARD			208									//广场赛奖励
#define DBR_GR_LOAD_SQUARE_REWARD		209									//加载广场赛奖励配置
...
#define DBR_GR_QUERY_TRANSRECORD    405                                 //转账记录
#define DBR_GR_QUERY_NICKNAME_BY_GAMEID		406							// 通过GameID查询昵称
#define DBR_GR_VERIFY_INSURE_PASSWORD		407							// 验证银行密码
#define DBR_GR_MODIFY_INSURE_PASSWORD		408							// 修改银行密码	
...
// 用户自建桌子
#define DBR_GR_USER_CREATE_CUSTOMTABLE			700						// 用户自建桌子
#define DBR_GR_USER_JOIN_CUSTOMTABLE			701						// 用户加入自定义桌子
#define DBR_GR_CONCLUDE_CUSTOMTABLE				702						// 解散自定义桌子
#define DBR_GR_USER_START_CUSTOMGAME			703						// 用户在自定义桌子上开始游戏
...
#define DBR_GR_TRANS_RECORD         304                                 //转账记录
#define DBO_GR_INSURE_TRANS_RECEIPT	305									// 转账回执
...
#define DBO_GR_BALANCE_SCORE_CURVE		602									//平衡分
#define DBO_GR_SQUARE_REWARD			603									//广场赛奖励信息
#define DBO_GR_LOAD_SQUARE_REWARD		604									//加载广场赛奖励配置信息

// 操作时间
#define DBO_GR_OPERATE_SUCCESS		700									// 操作成功
#define DBO_GR_OPERATE_FAILURE		701									// 操作失败

//用户事件
#define DBO_GR_USER_BALANCE_SCORE	800									// 用户平衡分

// 自定义桌子事件
#define DBO_GR_USER_CUSTOM_EVENT_START				900							// 用户自定义桌子起始标志
#define DBO_GR_CREATE_CUSTOMTABLE_SUCCESS				900							// 用户自定义桌子/加入自定义桌子成功
#define DBO_GR_CREATE_CUSTOMTABLE_FAILURE				901							// 用户自定义桌子/加入自定义桌子失败
#define DBO_GR_DISMISS_CUSTOMTABLE_SUCCESS			902							// 解散桌子成功
#define DBO_GR_DISMISS_CUSTOMTABLE_FAILURE			903							// 解散桌子失败
#define DBO_GR_JOIN_CUSTOMTABLE_SUCCESS				904							// 加入桌子成功
#define DBO_GR_JOIN_CUSTOMTABLE_FAILURE				905							// 加入桌子失败
#define DBO_GR_USER_ROUNDSCORE_IN_CUSTOMTABLE		906							// 玩家在一次自定义轮内的分数总计
#define DBO_GR_USER_CUSTOM_EVENT_END				950							// 用户自定义桌子起始标志
...
struct DBO_GR_LogonSuccess
{
	...
	// 玩家输赢(added by anjay)
	SCORE							lTotalWin;							// 玩家游戏总赢
	SCORE							lTotalLose;							// 玩家游戏总输
...
//	tagAndroidParameter				AndroidParameter[MAX_ANDROID];		//机器信息
	tagAndroidParameter				AndroidParameter;					// 机器人配置
};
...
{
	DWORD							dwUserID;							// 玩家的数据
	LONG							lBalanceScore;						// 玩家的平衡分
	LONG							lBuffTime;							// 平衡分Buff持续时间
	LONG							lBuffGames;							// 平衡分Buff持续游戏局数
	LONG							lBuffAmount;						// 平衡分Buff的量(正负都可以)
	bool							bLogon;								// 是否是登录时得到的平衡分
};

struct DBO_GR_UserRoundScoreInCustomTable
{
	DWORD							dwUserID;							// 玩家的数据
	SCORE							lRoundScore;						// 玩家积分
};

// 平衡分配置数据
struct DBO_GR_BalanceScoreCurve
{
	SCORE							lScore;								// 输赢总和的分数
	LONG							lBalanceScore;						// 对应的平衡分调整的分数
};
...
struct DBO_GR_Square_Reward
{
	...
	BYTE							cbMemberOrder;						//会员等级
	DWORD							dwUserRight;						//会员权限
	TCHAR							szAwardInfo[1024];					//广场赛奖励信息
};
...
struct DBR_GR_UserStartCustomGame
{
	...
	WORD							wTableID;							// 桌号
	WORD							wChairID;							// 椅子编号
	DWORD							dwRoundCount;						// 已进行的局数
};

// 自建桌子成功
struct DBO_GR_UserCustomTableSuccess
{
	DWORD							dwCreateUserID;						// 创建用户ID
	WORD							wKindID;							// 游戏类型
	WORD							wServerID;							// 房间编号
	WORD							wTableID;							// 桌子编号
	...
	BYTE							cbCustomRule[LEN_CUSTOMRULE];		// 自定规则
	SYSTEMTIME						tmCreateCustomTable;				// 创建时间
	DWORD							dwPropertyCount;					// 玩家剩余道具数量
};
...
{
	WORD							wTableID;							// 加入桌子成功,返回是否有加入过桌子
	WORD							wChairID;							// 加入桌子成功,返回是否有加入过桌子
...
{
	WORD							wTableID;							// 桌子编号
	WORD							wConcludeType;						// 解散类型
	DWORD							dwTableCreater;						// 桌子创建者
	DWORD							dwPropertyCount;					// 创建者剩余的钻石数量
};
