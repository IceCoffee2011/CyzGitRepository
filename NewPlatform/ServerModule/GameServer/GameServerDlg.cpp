...
VOID CGameServerDlg::OnBnClickedStopService()
{
	...
	try
	{
		...
		{
			// 自建桌子模式下,停止服务后直接关闭进程
			// 因为停止服务后,可以配置房间.但是关于自建桌子的相关数据并没有保存在ModuleManager中,配置完成后点击完成时,配置数据会被清零
			LPCTSTR pszQuestion = TEXT("自建房间模式中,停止服务后将会直接关闭进程，您确实要停止服务吗？");
...
VOID CGameServerDlg::OnBnClickedLoadSquareMatch()
{
	...
	for(int i=0; i<DlgServerItem.m_ModuleInitParameter.cbGameSquareOptionArrayCount; i++)
	{
		...
		{
			LOGW("CGameServerDlg::OnBnClickedLoadSquareMatch 加载广场赛错误, 此广场赛已结束 wMatch= "\
		...
		for(int j=0; j<m_ModuleInitParameter.cbGameSquareOptionArrayCount;j++)
		{
			...
			{//已存在广场赛
				...
				{
					LOGW("CGameServerDlg::OnBnClickedLoadSquareMatch 加载广场赛失败, 广场赛已开始 wMatch= "\
	...
	else
	{
		//构造提示
		TCHAR szString[256]=TEXT("");
		_sntprintf(szString,CountArray(szString),TEXT("无广场赛参数需要更新"));

		//输出信息
		CTraceService::TraceString(szString,TraceLevel_Normal);

		LOGI("CGameServerDlg::OnBnClickedLoadSquareMatch() 无广场赛参数需要更新!");
	}
};
//关闭询问
