...
typedef vector<CTableFrame*>			CTableFrameArray;				// 桌子数组
//typedef map<WORD, CTableFrame*>		CTableFrameArray;				// 桌子数组
...
protected:
	...
	tagGameSquareOption	*			m_pGameSquareOptionArray[20];		//广场赛配置
	BYTE							m_cbGameSquareOptionArrayCount;		//广场赛场次
...
public:
	CSquareManager					m_SquareManager;					//广场赛管理
	//组件接口
protected:
...
public:
	...
	//计算广场赛积分
	virtual BYTE ResultSquareMatchScore(ITableFrame* pITableFrame, tagScoreInfo ScoreInfoArray[], WORD wScoreCount);
	//计算广场赛积分
	virtual BYTE ResultSquareMatchScore(ITableFrame* pITableFrame, WORD wChairID, tagScoreInfo ScoreInfo);
...
protected:
	...
	//广场赛奖励信息
	bool OnDBSquareReward(DWORD dwContextID, VOID * pData, WORD wDataSize); 
	//加载广场赛奖励配置信息
	bool OnDBLoadSquareReward(DWORD dwContextID, VOID * pData, WORD wDataSize); 
	// 自建房间成功
	bool OnDBCreateCustomTableSuccess(DWORD dwContextID, VOID * pData, WORD wDataSize);
	// 自建房间失败
	bool OnDBCreateCustomTableFailed(DWORD dwContextID, VOID * pData, WORD wDataSize);
	// 解散桌子成功
	bool OnDBDismissCustomTableSuccess(DWORD dwContextID, VOID * pData, WORD wDataSize);
	// 解散桌子失败
	bool OnDBDismissCustomTableFailed(DWORD dwContextID, VOID * pData, WORD wDataSize);
	// 加入桌子成功
	bool OnDBJoinCustomTableSuccess(DWORD dwContextID, VOID * pData, WORD wDataSize);
	// 加入桌子失败
	bool OnDBJoinCustomTableFailed(DWORD dwContextID, VOID * pData, WORD wDataSize);
	// 玩家在自定义桌子上的分数
	bool OnDBUserRoundScoreInCustomTable(DWORD dwContextID, VOID * pData, WORD wDataSize);
...
protected:
	...
	// 用户自建桌子
	bool OnTCPNetworkMainUserCustom(WORD wSubCmdID, VOID * pData, WORD wDataSize, DWORD dwSocketID);
...
protected:
	// 自建桌子
	bool OnTCPSocketUserCreateCustomTable(VOID * pData, WORD wDataSize);
	// 加入自建桌子
	bool OnTCPNetworkUserJoinCustomTable(VOID * pData, WORD wDataSize, DWORD dwSocketID);
...
protected:
	...
	//版本检查
	bool PerformCheckVersion(DWORD dwPlazaVersion, DWORD dwFrameVersion, DWORD dwClientVersion, DWORD dwSocketID, bool bMobileClient = false);
	//切换连接
...
protected:
	...
	//配置广场赛
	bool InitSquareMatch();
	//更新广场赛
	bool UpdateSquareMatch(tagGameSquareOption* pGameSquareOption);
	//发送请求
...
protected:
	//评估机器人座在哪桌
	WORD GetAndriodTable(IAndroidUserItem * pIAndroidUserItem);
};
