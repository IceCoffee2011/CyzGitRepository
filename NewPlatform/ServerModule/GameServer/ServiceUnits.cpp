	...
	for(int i=0; i<cbGameSquareOptionArrayCount; i++)
	{
		...
		for(int j=0; j<m_vGameSquareOption.size();j++)
		{
			...
			{//已存在广场赛
				...
				{
					LOGW("CServiceUnits::UpdateSquareMatch 加载广场赛失败, 广场赛已开始 wMatch= "\
...
// 				memcpy(&(m_vGameSquareOption[j]), &(DlgServerItem.m_ModuleInitParameter.GameSquareOptionArray[i]),sizeof(tagGameSquareOption));
	...
	for(int i=0; i<cbGameSquareOptionArrayCount; i++)
	{
		...
		{
			LOGW("CServiceUnits::UpdateSquareMatch 加载广场赛错误, 此广场赛已结束 wMatch= "\
...
// 		LOGI("CServiceUnits::UpdateSquareMatch 加载广场赛 wMatch= "\
...
// 			<< m_AttemperEngineSink.m_pGameSquareOptionArray[i]->szSquareName);
	...
	for(map<WORD,CSquare>::iterator iter=m_AttemperEngineSink.m_SquareManager.begin(); iter!=m_AttemperEngineSink.m_SquareManager.end(); iter++)
	{
		...
		LOGI("CServiceUnits::UpdateSquareMatch 更新广场赛配置: MatchID= " << iter->second.m_pGameSquareOption->wMatchID\
		...
		LOGI("CServiceUnits::UpdateSquareMatch 广场赛奖励配置 RankBonusCount= " << iter->second.m_pGameSquareOption->cbRankBonus);
...
// 		}
	...
	if (m_GameMatchServiceManager.GetInterface()!=NULL)
	{
		...
		{
			CTraceService::TraceString(TEXT("比赛服务调整配置异常"), TraceLevel_Exception);
			return false;
		}
	}
