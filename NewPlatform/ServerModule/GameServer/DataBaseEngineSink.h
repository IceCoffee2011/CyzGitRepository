...
protected:
	...
	// 用户自定义数据库
protected:
	CDataBaseAide					m_UserCustomDBAide;					//平台数据库
	CDataBaseHelper					m_UserCustomDBModule;				//平台数据库
...
protected:
	...
	bool OnRequestLoadBalanceScoreCurve(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID);
	// 重置游戏锁
	bool OnRequestResetGameScoreLocker(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID);

	//广场赛奖励
	bool OnRequestSquareReward(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID);
	//加载广场赛奖励配置
	bool OnRequestLoadSquareReward(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID);
...
protected:
    ...
    //转账记录
	bool OnRequestQueryTransRecord(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID);
	//查询用户
	bool OnRequestQueryTransferUserInfo(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID);
	// 根据GameID查询昵称
	bool OnRequestQueryNickNameByGameID(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID);
	// 验证银行密码
	bool OnRequestVerifyInsurePassword(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID);
	// 更改银行密码
	bool OnRequestModifyInsurePassword(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID);

	// 用户自定义桌子
protected:
	// 用户创建桌子
	bool OnRequestUserCreateCustomTable(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID);
	// 用户加入桌子
	bool OnRequestUserJoinCustomTable(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID);
	// 解散自定义桌子
	bool OnRequestConcludeCustomTable(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID);
	// 在自定义桌子开始游戏
	bool OnRequestUserStartCustomGame(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID);
...
private:
	...
	// 平衡分结果
	VOID OnUserBalanceScoreDisposeResult(DWORD dwContextID, DWORD dwUserID, bool bLogon = true);
	// 自建桌子结果
	VOID OnUserCustomTableDisposeResult(DWORD dwContextID, DWORD dwErrorCode, LPCTSTR pszErrorString, LPBYTE pcbCustomRule, WORD wTableID);
	// 加入桌子结果
	VOID OnUserJoinCustomTableDisposeResult(DWORD dwContextID, DWORD dwErrorCode, LPCTSTR pszErrorString);
	// 解散桌子结果
	VOID OnDismissCustomTableDisposeResult(DWORD dwContextID, DWORD dwErrorCode, LPCTSTR pszErrorString, WORD wTableID, WORD wConcludeType);
	// 一次房卡内,玩家的累积分数
	VOID OnUserInCustomTableScoreDisposeResult(DWORD dwContextID, DWORD dwUserID);
