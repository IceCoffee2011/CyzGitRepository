...
protected:
	...
	// 自建桌子变量
protected:
	bool							m_bReset;							// 重置标志,用于自建桌子
	bool							m_bLockedForCustom;					// 创建桌子时,先锁定
	bool							m_bLockedForDismiss;				// 解散桌子时,先锁定
	map<WORD, bool>					m_mapDismissVote;					// 解散投票结果
	WORD							m_wConcludeType;					// 解散类型
...
protected:
	...
	tagGameTableOption*				m_pGameTableOption;
...
public:
	...
	// 获取当前局数(局数从0开始)
	virtual WORD GetDrawCount() { return m_wDrawCount; };

	//配置参数
public:
	//自定配置
	virtual VOID * GetCustomRule() { return m_pGameTableOption->cbCustomRule; };
	//服务属性
	...
	//桌子配置
	virtual tagGameTableOption*	GetGameTableOption();
...
public:
	...
	//旁观动作
	virtual bool PerformLookonAction(WORD wChairID, IServerUserItem * pIServerUserItem, LPCTSTR lpszPassword=NULL);
	//坐下动作
	virtual bool PerformSitDownAction(WORD wChairID, IServerUserItem * pIServerUserItem, LPCTSTR lpszPassword=NULL);
	// 托管退出模式下站起
	virtual bool PerformStandUpActionInTrusteeMode(IServerUserItem * pIServerUserItem);
...
public:
	// 玩家申请解散
	bool OnEventSocketFrameApplyForDismiss(VOID * pData, WORD wDataSize, IServerUserItem * pIServerUserItem);
	// 玩家解散事件的投票结果
	bool OnEventSocketFrameChoiceForDismiss(VOID * pData, WORD wDataSize, IServerUserItem * pIServerUserItem);
...
public:
	...
	// 权限校验
	bool EfficacyEnterTableMemberRule(IServerUserItem * pIServerUserItem);
...
public:
	...
	// 设置密码
	void SetSecret(string strSecret) { lstrcpyn(m_pGameTableOption->szSecret, strSecret.c_str(), sizeof(m_pGameTableOption->szSecret)); };
	// 设置自定义局数
	void SetCustomRoundCount(DWORD dwRoundCount) { m_pGameTableOption->dwRoundCount = dwRoundCount; };
	// 设置自定义桌子创建时间

	// 获取桌子绑定的密码
	string GetSecret() { return string(m_pGameTableOption->szSecret); };
	// 设置自定义锁定
	void SetLockForCustom(bool bLocked) { m_bLockedForCustom = bLocked; };
	// 桌子是否被锁定
	bool IsLockedForCustom() { return m_bLockedForCustom; };
	// 重置桌子数据
	void Reset();
	// 设置解散锁定
	void SetLockForDismiss(bool bLocked) { m_bLockedForDismiss = bLocked; };
	// 检查解散桌子相关投票
	void CheckVoteForDismiss();
	// 解散自建桌子
	void DismissCustomTable(WORD wConcludeType);
	// 检查是否超时,解散桌子
	bool CheckCustomTableOverdue();
};
