...
#define ITEM_SERVER_OPTION_4		3									//配置选择
...
BEGIN_MESSAGE_MAP(CDlgServerOptionItem1, CDlgServerOptionItem)
	...
	ON_BN_CLICKED(IDC_BTN_TABLE_OPTION, &CDlgServerOptionItem1::OnBnClickedBtnTableOption)
END_MESSAGE_MAP()
...
BEGIN_MESSAGE_MAP(CDlgServerOptionItemCustom, CDlgServerOptionItem)
	ON_WM_SETFOCUS()
	ON_WM_NCDESTROY()
	ON_BN_CLICKED(IDOK, &CDlgServerOptionItemCustom::OnBnClickedOk)
	ON_BN_CLICKED(IDCANCEL, &CDlgServerOptionItemCustom::OnBnClickedCancel)
END_MESSAGE_MAP()
...
BOOL CDlgServerOptionItem1::OnInitDialog()
{
	...
	if (m_pGameServiceOption->wServerType&GAME_GENRE_USERCUSTOM)
	{
		...
		{
			SetDlgItemInt(IDC_TABLE_COUNT, 0);
		}
	}
	else
	{
...
//		((CIPAddressCtrl *)GetDlgItem(IDC_DATABASE_ADDR))->SetAddress(htonl(dwDataBaseAddr));
		SetDlgItemText(IDC_DATABASE_ADDR, m_pGameServiceOption->szDataBaseAddr);
	}
...
//	CIPAddressCtrl * pIPAddressCtrl=(CIPAddressCtrl *)GetDlgItem(IDC_DATABASE_ADDR);
	...
	GetDlgItemText(IDC_DATABASE_ADDR, m_pGameServiceOption->szDataBaseAddr, CountArray(m_pGameServiceOption->szDataBaseAddr));
...
// 	{
// 		tagGameTableInfo* pGameOption = m_pGameTableOption + i;
// 		if (pGameOption == NULL)
// 		{
// 			continue;
// 		}
// 
// 		if (pGameOption->wEndTableID > maxtableid)
// 			maxtableid = pGameOption->wEndTableID;
// 	}
	for (size_t i = 0; i < m_pVectGameTableInfo->size(); i++)
	{
		tagDBGameTableInfo GameTableInfo = m_pVectGameTableInfo->at(i);

		if (GameTableInfo.wEndTableID > maxtableid)
			maxtableid = GameTableInfo.wEndTableID;
	}

// 	if (maxtableid == 0)
// 	{
// 		AfxMessageBox(TEXT("还未配置桌子信息"), MB_ICONERROR);
// 		return false;
// 	}

	if (maxtableid < m_pGameServiceOption->wTableCount)
	{
		AfxMessageBox(TEXT("配置的桌子信息比桌子数目要少"), MB_ICONERROR);
		return false;
	}

	if (maxtableid > m_pGameServiceOption->wTableCount)
	{
		if (AfxMessageBox(TEXT("配置的桌子信息比桌子数目要多,请注意是否正常!!"), MB_YESNO) == IDNO)
		{
			return false;
		}
	}
...
// 	{
// 		AfxMessageBox(TEXT("游戏桌子数目无效，请重新输入游戏桌子数目，有效数值范围为 1 - 100"),MB_ICONERROR);
// 		return false;
// 	}
	...
	if (m_pGameServiceOption->wServerPort == 0)
	{
		AfxMessageBox(TEXT("游戏服务端口不能为空, 请重新输入服务端口，有效数值范围为 1 - 65535"), MB_ICONERROR);
		return false;
	}
...
VOID CDlgServerOptionItem1::InitCtrlWindow()
{
	...
	//常规配置
	((CEdit *)GetDlgItem(IDC_TABLE_COUNT))->LimitText(5);
	...
	WORD wServerType[5]={GAME_GENRE_GOLD, GAME_GENRE_SCORE, GAME_GENRE_MATCH, GAME_GENRE_EDUCATE, GAME_GENRE_USERCUSTOM};
	LPCTSTR pszServerType[5] = { TEXT("财富类型"), TEXT("点值类型"), TEXT("比赛类型"), TEXT("练习类型"), TEXT("自建桌子类型") };
...
VOID CDlgServerOptionItem1::UpdateDataBaseName()
{
	...
	//数据调整
	switch (pServerType->GetItemData(pServerType->GetCurSel()))
	{
	...
	case GAME_GENRE_USERCUSTOM:	// 自建桌子类型
		{
			SetDlgItemText(IDC_DATABASE_NAME, szUserCustomDB);
			break;
		}
	default:
		{
...
void CDlgServerOptionItem1::OnBnClickedBtnTableOption()
{
	// 点击 桌子配置 按钮
//	CDlgRoomTable dlg(&m_vectTableOption);
	CDlgRoomTable dlg(m_pDlgServerWizard, m_pVectGameTableInfo);
	if (dlg.DoModal() == IDOK)
	{
	};
}
...
BOOL CDlgServerOptionItem2::OnInitDialog()
{
	...
	//所有财富游戏，每局即时写分，记录每局游戏币，默认为直接勾选灰色状态
	...
	// 禁止房间取钱
	bool bForfendTakeInRoom=CServerRule::IsForfendTakeInRoom(m_pGameServiceOption->dwServerRule);
	((CButton *)GetDlgItem(IDC_FORFEND_TAKE_IN_ROOM))->SetCheck((bForfendTakeInRoom==TRUE)?BST_CHECKED:BST_UNCHECKED);

	// 禁止游戏取钱
	bool bForfendTakeInGame=CServerRule::IsForfendTakeInGame(m_pGameServiceOption->dwServerRule);
	((CButton *)GetDlgItem(IDC_FORFEND_TAKE_IN_GAME))->SetCheck((bForfendTakeInGame==TRUE)?BST_CHECKED:BST_UNCHECKED);

	// 禁止房间转账
	bool bForfendTransferInRoom=CServerRule::IsForfendTransferInRoom(m_pGameServiceOption->dwServerRule);
	((CButton *)GetDlgItem(IDC_FORFEND_TRANSFER_IN_ROOM))->SetCheck((bForfendTransferInRoom==TRUE)?BST_CHECKED:BST_UNCHECKED);

	// 禁止游戏转账
	bool bForfendTransferInGame=CServerRule::IsForfendTransferInGame(m_pGameServiceOption->dwServerRule);
	((CButton *)GetDlgItem(IDC_FORFEND_TRANSFER_IN_GAME))->SetCheck((bForfendTransferInGame==TRUE)?BST_CHECKED:BST_UNCHECKED);
	...
	// 禁止计算运气值
//	bool bForfendCalculateBalanceScore=CServerRule::IsForfendCalculateBalanceScore(m_pGameServiceOption->dwServerRule);
//	((CButton *)GetDlgItem(IDC_FORFEND_CALCULATE_BALANCE_SCORE))->SetCheck((bForfendCalculateBalanceScore==TRUE)?BST_CHECKED:BST_UNCHECKED);

	// 禁止获取运气值
//	bool bForfendGetBalanceScore=CServerRule::IsForfendGetBalanceScore(m_pGameServiceOption->dwServerRule);
//	((CButton *)GetDlgItem(IDC_FORFEND_GET_BALANCE_SCORE))->SetCheck((bForfendGetBalanceScore==TRUE)?BST_CHECKED:BST_UNCHECKED);
	...
	if (m_pGameServiceAttrib->cbOffLineTrustee==TRUE)
	{
		...
		GetDlgItem(IDC_ALLOW_TRUSTEE_WRITESCORE)->EnableWindow(TRUE);
		bool bAllowTrusteeWriteScore = CServerRule::IsAllowTrusteeWriteScore(m_pGameServiceOption->dwServerRule);
		((CButton *)GetDlgItem(IDC_ALLOW_TRUSTEE_WRITESCORE))->SetCheck((bAllowTrusteeWriteScore == TRUE) ? BST_CHECKED : BST_UNCHECKED);
	}
	else
	{
		GetDlgItem(IDC_ALLOW_TRUSTEE_WRITESCORE)->EnableWindow(FALSE);
	}
...
bool CDlgServerOptionItem2::SaveInputInfo()
{
	...
	{
		AfxMessageBox(TEXT("进入房间最低游戏币比最高游戏币还高，将会造成任何玩家都不能进入"),MB_ICONERROR);
		return false;
	}
	...
	// 禁止房间取钱
	CButton * pForfendTakeInRoom=(CButton *)GetDlgItem(IDC_FORFEND_TAKE_IN_ROOM);
	CServerRule::SetForfendTakeInRoom(m_pGameServiceOption->dwServerRule,(pForfendTakeInRoom->GetCheck()==BST_CHECKED));

	// 禁止游戏取钱
	CButton * pForfendTakeInGame=(CButton *)GetDlgItem(IDC_FORFEND_TAKE_IN_GAME);
	CServerRule::SetForfendTakeInGame(m_pGameServiceOption->dwServerRule,(pForfendTakeInGame->GetCheck()==BST_CHECKED));

	// 禁止房间转账
	CButton * pForfendTransferInRoom=(CButton *)GetDlgItem(IDC_FORFEND_TRANSFER_IN_ROOM);
	CServerRule::SetForfendTransferInRoom(m_pGameServiceOption->dwServerRule,(pForfendTransferInRoom->GetCheck()==BST_CHECKED));

	// 禁止游戏转账
	CButton * pForfendTransferInGame=(CButton *)GetDlgItem(IDC_FORFEND_TRANSFER_IN_GAME);
	CServerRule::SetForfendTransferInGame(m_pGameServiceOption->dwServerRule,(pForfendTransferInGame->GetCheck()==BST_CHECKED));
	...
	// 禁止结算运气值
//	CButton * pForfendCalculateBalanceScore=(CButton *)GetDlgItem(IDC_FORFEND_CALCULATE_BALANCE_SCORE);
//	CServerRule::SetForfendCalculateBalanceScore(m_pGameServiceOption->dwServerRule,(pForfendCalculateBalanceScore->GetCheck()==BST_CHECKED));

	// 禁止获取运气值
//	CButton * pForfendGetBalanceScore=(CButton *)GetDlgItem(IDC_FORFEND_GET_BALANCE_SCORE);
//	CServerRule::SetForfendGetBalanceScore(m_pGameServiceOption->dwServerRule,(pForfendGetBalanceScore->GetCheck()==BST_CHECKED));
	...
	if (m_pGameServiceAttrib->cbOffLineTrustee==TRUE)
	{
		...
		CButton * pAllowTrusteeWriteScore = (CButton *)GetDlgItem(IDC_ALLOW_TRUSTEE_WRITESCORE);
		CServerRule::SetAllowTrusteeWriteScore(m_pGameServiceOption->dwServerRule, (pAllowTrusteeWriteScore->GetCheck() == BST_CHECKED));
	}
...
VOID CDlgServerOptionItem2::InitCtrlWindow()
{
	...
	GetDlgItem(IDC_ALLOW_TRUSTEE_WRITESCORE)->EnableWindow((m_pGameServiceAttrib->cbOffLineTrustee == TRUE) ? TRUE : FALSE);
...
//构造函数
CDlgServerOptionItem4::CDlgServerOptionItem4() : CDlgServerOptionItem(IDD_SERVER_OPTION_5)
{
}
...
BOOL CDlgServerOptionItem4::OnInitDialog()
{
	...
	//变量定义
	CComboBox * pSquareSortType=(CComboBox *)GetDlgItem(IDC_SQUARE_SORT_TYPE);

	//游戏类型
	for (INT i=0;i<pSquareSortType->GetCount();i++)
	{
		if (pSquareSortType->GetItemData(i)==/*m_pGameServiceOption->wServerType*/1)
		{
			pSquareSortType->SetCurSel(i);
		}
	}

	//默认选择
	if (pSquareSortType->GetCurSel()==LB_ERR)
	{
		for (INT i=0;i<pSquareSortType->GetCount();i++)
		{
			if (pSquareSortType->GetItemData(i)==1)
			{
				pSquareSortType->SetCurSel(i);
				break;
			}
	...
	//!排序类型暂时没有使用
	pSquareSortType->EnableWindow(SW_HIDE);
...
{
	//变量定义
	CDateTimeCtrl * pSquareSartDate=(CDateTimeCtrl *)GetDlgItem(IDC_DATETIMEPICKER_START_DATE);
	CDateTimeCtrl * pSquareSartTime=(CDateTimeCtrl *)GetDlgItem(IDC_DATETIMEPICKER_START_TIME);
	CDateTimeCtrl * pSquareEndDate=(CDateTimeCtrl *)GetDlgItem(IDC_DATETIMEPICKER_END_DATE);
	CDateTimeCtrl * pSquareEndTime=(CDateTimeCtrl *)GetDlgItem(IDC_DATETIMEPICKER_END_TIME);
	CButton * pStartSquareMatch=(CButton *)GetDlgItem(IDC_START_SQUAREMATCH);
	BOOL bStartSquareMatch=pStartSquareMatch->GetCheck()==BST_CHECKED;

	if(bStartSquareMatch)
	{
		//开始时间
		CTime timeStartDate;
		CTime timeStartTime;
		DWORD dwResult=pSquareSartDate->GetTime(timeStartDate);
		if(dwResult!=GDT_VALID)
		{
			if(dwResult==GDT_NONE)AfxMessageBox(TEXT("广场赛开始日期不能为空，请输入广场赛开始日期"),MB_ICONERROR);
			if(dwResult==GDT_ERROR)AfxMessageBox(TEXT("广场赛开始日期错误，请重新输入广场赛开始日期"),MB_ICONERROR);
			return false;
		}
		//TCHAR szDate[128]=TEXT("");
		//_sntprintf(szDate,sizeof(szDate)-1,TEXT("%d-%d-%d %d:%d:%d"),timeStartDate.GetYear(),timeStartDate.GetMonth(),timeStartDate.GetDay(),\
		//	timeStartDate.GetHour(),timeStartDate.GetMinute(),timeStartDate.GetSecond());
		//AfxMessageBox(szDate,MB_ICONERROR);

		dwResult=pSquareSartTime->GetTime(timeStartTime);
		if(dwResult!=GDT_VALID)
		{
			if(dwResult==GDT_NONE)AfxMessageBox(TEXT("广场赛开始时间不能为空，请输入广场赛开始时间"),MB_ICONERROR);
			if(dwResult==GDT_ERROR)AfxMessageBox(TEXT("广场赛开始时间错误，请重新输入广场赛开始时间"),MB_ICONERROR);
			return false;
		}
		//memset(szDate,0,sizeof(szDate));
		//_sntprintf(szDate,sizeof(szDate)-1,TEXT("%d-%d-%d %d:%d:%d"),timeStartTime.GetYear(),timeStartTime.GetMonth(),timeStartTime.GetDay(),\
		//	timeStartTime.GetHour(),timeStartTime.GetMinute(),timeStartTime.GetSecond());
		//AfxMessageBox(szDate,MB_ICONERROR);

		CTime timeStart(timeStartDate.GetYear(),timeStartDate.GetMonth(),timeStartDate.GetDay(),\
			timeStartTime.GetHour(),timeStartTime.GetMinute(),timeStartTime.GetSecond());
		//memset(szDate,0,sizeof(szDate));
		//_sntprintf(szDate,sizeof(szDate)-1,TEXT("%d-%d-%d %d:%d:%d"),timeStartDate.GetYear(),timeStartDate.GetMonth(),timeStartDate.GetDay(),\
		//	timeStartTime.GetHour(),timeStartTime.GetMinute(),timeStartTime.GetSecond());
		...
		CTime timeEndDate;
		CTime timeEndTime;
		dwResult=pSquareEndDate->GetTime(timeEndDate);
		if(dwResult!=GDT_VALID)
		{
			if(dwResult==GDT_NONE)AfxMessageBox(TEXT("广场赛结束日期不能为空，请输入广场赛结束日期"),MB_ICONERROR);
			if(dwResult==GDT_ERROR)AfxMessageBox(TEXT("广场赛结束日期错误，请重新输入广场赛结束日期"),MB_ICONERROR);
			return false;
		}
		//memset(szDate,0,sizeof(szDate));
		//_sntprintf(szDate,sizeof(szDate)-1,TEXT("%d-%d-%d %d:%d:%d"),timeEndDate.GetYear(),timeEndDate.GetMonth(),timeEndDate.GetDay(),\
		//	timeEndDate.GetHour(),timeEndDate.GetMinute(),timeEndDate.GetSecond());
		//AfxMessageBox(szDate,MB_ICONERROR);

		dwResult=pSquareEndTime->GetTime(timeEndTime);
		if(dwResult!=GDT_VALID)
		{
			if(dwResult==GDT_NONE)AfxMessageBox(TEXT("广场赛结束时间不能为空，请输入广场赛结束时间"),MB_ICONERROR);
			if(dwResult==GDT_ERROR)AfxMessageBox(TEXT("广场赛结束时间错误，请重新输入广场赛结束时间"),MB_ICONERROR);
			return false;
		}
		//memset(szDate,0,sizeof(szDate));
		//_sntprintf(szDate,sizeof(szDate)-1,TEXT("%d-%d-%d %d:%d:%d"),timeEndTime.GetYear(),timeEndTime.GetMonth(),timeEndTime.GetDay(),\
		//	timeEndTime.GetHour(),timeEndTime.GetMinute(),timeEndTime.GetSecond());
		//AfxMessageBox(szDate,MB_ICONERROR);
		...
		{
			AfxMessageBox(TEXT("广场赛开始日期时间与结束日期时间冲突，请重新输入广场赛开始结束日期时间"),MB_ICONERROR);
			return false;
		}

		WORD wInterval=GetDlgItemInt(IDC_TIME_INTERVAL);
		if(wInterval<=0||wInterval>=10)
		{
			AfxMessageBox(TEXT("广场赛消息时间间隔超出范围，请重新输入广场赛消息时间间隔"),MB_ICONERROR);
			return false;
		}

		//排名限额
		WORD wRankNumber=GetDlgItemInt(IDC_RANK_NUMBER);
		if(wRankNumber<=0||wRankNumber>=1000)
		{
			AfxMessageBox(TEXT("广场赛排名限额超出范围，请重新输入广场赛排名限额"),MB_ICONERROR);
			return false;
		}

		//获取名字
		TCHAR szSquareMatchName[1024]=TEXT("");
		GetDlgItemText(IDC_SQUAREMATCH_NAME,szSquareMatchName,CountArray(szSquareMatchName));
		if(lstrlen(szSquareMatchName)<=0||lstrlen(szSquareMatchName)>=1024)
		{
			AfxMessageBox(TEXT("广场赛名称超出范围，请重新输入广场赛名称"),MB_ICONERROR);
			return false;
		}
		
		//获取名字
		TCHAR szSquareMatchDesc[1024]=TEXT("");
		GetDlgItemText(IDC_SQUAREMATCH_DESC,szSquareMatchDesc,CountArray(szSquareMatchDesc));
		if(lstrlen(szSquareMatchDesc)<=0||lstrlen(szSquareMatchDesc)>=1024)
		{
			AfxMessageBox(TEXT("广场赛描述超出范围，请重新输入广场赛描述"),MB_ICONERROR);
			return false;
		}

		//属性配置
		m_pGameSquareOption->cbStartSquareMatch=bStartSquareMatch;
		m_pGameSquareOption->wRankNumber=wRankNumber;
		m_pGameSquareOption->wInterval=wInterval;
		timeStart.GetAsSystemTime(m_pGameSquareOption->StartTime);
		timeEnd.GetAsSystemTime(m_pGameSquareOption->EndTime);
		CopyMemory(m_pGameSquareOption->szSquareName,szSquareMatchName,sizeof(m_pGameSquareOption->szSquareName));
		CopyMemory(m_pGameSquareOption->szSquareDesc,szSquareMatchDesc,sizeof(m_pGameSquareOption->szSquareDesc));
	}
...
//构造控件
VOID CDlgServerOptionItem4::InitCtrlWindow()
{
	//时间控制
	((CEdit *)GetDlgItem(IDC_TIME_INTERVAL))->LimitText(1);

	//排名控制
	((CEdit *)GetDlgItem(IDC_RANK_NUMBER))->LimitText(3);

	//变量定义
	CComboBox * pComboBox=(CComboBox *)GetDlgItem(IDC_SQUARE_SORT_TYPE);
	WORD wSortType=1;
	LPCTSTR pszSortType=TEXT("普通排序");

	//构造类型
	for (INT i=0;i<1;i++)
	{
		pComboBox->SetItemData(pComboBox->AddString(pszSortType),wSortType);
	}
...
CDlgServerOptionItemCustom::CDlgServerOptionItemCustom() : CDlgServerOptionItem(IDD_SERVER_OPTION_4)
{
	...
	m_pGameTableInfo = nullptr;

	return;
}

CDlgServerOptionItemCustom::CDlgServerOptionItemCustom(CDlgServerWizard* pDlgServerWizard, tagDBGameTableInfo* pTableConfig) : CDlgServerOptionItem(IDD_SERVER_OPTION_4)
{
	//设置变量
	m_hCustomRule = NULL;
	m_pIGameServiceCustomRule = NULL;
	m_pDlgServerWizard = pDlgServerWizard;
	m_pGameTableInfo = pTableConfig;
...
#else
	WORD wCustomSize = sizeof(m_pGameTableInfo->TableOption.cbCustomRule);
 	if (m_pIGameServiceCustomRule != NULL)
 	{
		m_hCustomRule = m_pIGameServiceCustomRule->CreateCustomRule(this, rcClient, m_pGameTableInfo->TableOption.cbCustomRule, wCustomSize);
 	}
#endif
...
{
	//保存输入
	if (m_hCustomRule != NULL && m_pIGameServiceCustomRule != NULL)
	{
		//保存设置
#ifndef _READ_TABLE_OPTION_
		WORD wCustonSize=sizeof(m_pGameServiceOption->cbCustomRule);
		...
		return bSuccess;
#else
		WORD wCustonSize = sizeof(m_pGameTableInfo->TableOption.cbCustomRule);
		bool bSuccess = m_pIGameServiceCustomRule->SaveCustomRule(m_pGameTableInfo->TableOption.cbCustomRule, wCustonSize);
		return bSuccess;
#endif
	}
...
VOID CDlgServerOptionItemCustom::OnNcDestroy()
{
	...
	// 释放窗体指针
	if (m_pIGameServiceCustomRule)
	{
		m_pIGameServiceCustomRule->DestoryCustomRule();
	}
...
void CDlgServerOptionItemCustom::OnBnClickedOk()
{
	// TODO:  在此添加控件通知处理程序代码
	if (SaveInputInfo() == false)
	{
		// 保存配置失败,则需要继续配置
		return;
	}
...
void CDlgServerOptionItemCustom::OnBnClickedCancel()
{
	// TODO:  在此添加控件通知处理程序代码
	CDialog::OnCancel();
}
...
bool CDlgServerWizardItem1::SaveInputInfo()
{
	...
	if (m_GameServiceManager.CreateInstance()==false)
	{
		//构造提示
		TCHAR szString[512]=TEXT("");
		_sntprintf(szString,CountArray(szString),TEXT("[ %s ] 服务组件[%s]，创建游戏房间失败"),pGameModuleInfo->szGameName, m_GameServiceManager.GetErrorDescribe());
...
BOOL CDlgServerWizardItem2::OnInitDialog()
{
	...
	m_TabCtrl.InsertItem(ITEM_SERVER_OPTION_4,TEXT("广场赛配置"));
...
bool CDlgServerWizardItem2::ActiveOptionItem(WORD wIndex)
{
	...
	m_pOptionItem[wIndex]->m_pGameSquareOption=m_pGameSquareOption;
//	m_pOptionItem[wIndex]->m_pGameTableOption = m_pGameTableOption;
	m_pOptionItem[wIndex]->m_pVectGameTableInfo = m_pVectGameTableInfo;
...
//设置接口
VOID CDlgServerWizard::SetWizardParameter(IGameServiceManager * pIGameServiceManager, tagGameServiceOption * pGameServiceOption, vector<tagDBGameTableInfo>* pVectGameTableInfo /* = NULL */)
{
	//销毁子项
	...
	if (pGameServiceOption==NULL)
	{
		...
		for (size_t i = 0; i < m_ModuleInitParameter.GameServiceTableOption.TableOption.size(); i++)
		{
			m_pIGameServiceManager->RectifyParameter(m_ModuleInitParameter.GameServiceOption, m_ModuleInitParameter.GameServiceTableOption.TableOption.at(i));
		}
	}
	else
	{
	...
	// 设置桌子规则
	if (pVectGameTableInfo == nullptr)
	{
	}
	else
	{
		//拷贝规则
//		CopyMemory(&m_ModuleInitParameter.TableConfig, pGameTableOption, sizeof(m_ModuleInitParameter.TableConfig));
		for (size_t i = 0; i < pVectGameTableInfo->size(); i++)
		{
			m_ModuleInitParameter.TableConfig.push_back(pVectGameTableInfo->at(i));
		}
	}
...
bool CDlgServerWizard::ActiveWizardItem(WORD wIndex)
{
	...
	////切换至步骤2时需加载广场赛，是为了配置广场赛之前就加载好已存在的广场赛
	//if(wIndex==1&&m_ModuleInitParameter.GameServiceOption.wServerID!=0)
	//需加载广场赛
	if(m_ModuleInitParameter.GameServiceOption.wServerID!=0)
	{
	...
	//m_pWizardItem[wIndex]->m_pGameSquareOption=&m_ModuleInitParameter.GameSquareOption;
	WORD wSquareIndex=m_ModuleInitParameter.cbGameSquareOptionArrayCount>=20?19:m_ModuleInitParameter.cbGameSquareOptionArrayCount;
	m_pWizardItem[wIndex]->m_pGameSquareOption=&m_ModuleInitParameter.GameSquareOptionArray[wSquareIndex];
//	m_pWizardItem[wIndex]->m_pGameTableOption = &m_ModuleInitParameter.TableConfig[0];
	m_pWizardItem[wIndex]->m_pVectGameTableInfo = &(m_ModuleInitParameter.TableConfig);
...
VOID CDlgServerWizard::OnBnClickedFinish()
{
	...
	tagGameSquareOption SquareMatchResult;
	CServerInfoManager ServerInfoManager;
	...
	//添加广场赛
	WORD wSquareIndex=m_ModuleInitParameter.cbGameSquareOptionArrayCount>=20?19:m_ModuleInitParameter.cbGameSquareOptionArrayCount;
	if(m_ModuleInitParameter.GameSquareOptionArray[wSquareIndex].cbStartSquareMatch)
	{
		tagGameSquareOption GameSquareOption;
		ZeroMemory(&GameSquareOption,sizeof(GameSquareOption));

		//启动索引
		GameSquareOption.cbStartSquareMatch=m_ModuleInitParameter.GameSquareOptionArray[wSquareIndex].cbStartSquareMatch;

		//索引变量
		GameSquareOption.wGameID=m_ModuleInitParameter.GameServiceAttrib.wKindID;

		//房间属性
		GameSquareOption.wServerID=GameServerResult.wServerID;
		lstrcpyn(GameSquareOption.szServerName,m_ModuleInitParameter.GameSquareOptionArray[wSquareIndex].szServerName,CountArray(GameSquareOption.szServerName));

		//时间配置
		GameSquareOption.StartTime=m_ModuleInitParameter.GameSquareOptionArray[wSquareIndex].StartTime;
		GameSquareOption.EndTime=m_ModuleInitParameter.GameSquareOptionArray[wSquareIndex].EndTime;
		GameSquareOption.wInterval=m_ModuleInitParameter.GameSquareOptionArray[wSquareIndex].wInterval;

		//排名奖励配置
		GameSquareOption.wRankNumber=m_ModuleInitParameter.GameSquareOptionArray[wSquareIndex].wRankNumber;
		lstrcpyn(GameSquareOption.szSquareName,m_ModuleInitParameter.GameSquareOptionArray[wSquareIndex].szSquareName,CountArray(GameSquareOption.szSquareName));
		lstrcpyn(GameSquareOption.szSquareDesc,m_ModuleInitParameter.GameSquareOptionArray[wSquareIndex].szSquareDesc,CountArray(GameSquareOption.szSquareDesc));
		CopyMemory(GameSquareOption.RankBonus,m_ModuleInitParameter.GameSquareOptionArray[wSquareIndex].RankBonus,sizeof(GameSquareOption.RankBonus));

		//插入广场赛
		if (ServerInfoManager.InsertSquareMatch(&GameSquareOption,SquareMatchResult)==false) return;

		//提示消息
		CTraceService::TraceString(TEXT("广场赛配置成功"),TraceLevel_Normal);
	}

	//插入桌子配置
	bool flag = ServerInfoManager.InsertRoomTable(GameServerResult.wServerID, &(m_ModuleInitParameter.TableConfig));
	if (flag == false) 
	{
		//提示消息
		CTraceService::TraceString(TEXT("游戏房间桌子配置写表失败"), TraceLevel_Normal);
		return;
	}

	// 读取自建桌子信息 anjay to do
	...
	//税收配置
	// 此处给GameServiceOption赋正确值
	m_ModuleInitParameter.GameServiceTableOption.TableOption.clear();
	for (int i = 0; i < m_ModuleInitParameter.TableConfig.size(); i++)
	{
		WORD wStartTableID = m_ModuleInitParameter.TableConfig.at(i).wStartTableID;
		WORD wEndTableID = m_ModuleInitParameter.TableConfig.at(i).wEndTableID;

		// 桌号从1开始
		for (int j = wStartTableID; j <= wEndTableID; j++)
		{
			m_ModuleInitParameter.GameServiceTableOption.TableOption.push_back(m_ModuleInitParameter.TableConfig.at(i).TableOption);
			size_t TableIndex = m_ModuleInitParameter.GameServiceTableOption.TableOption.size() - 1;
			m_ModuleInitParameter.GameServiceTableOption.TableOption.at(TableIndex).wTableID = j - 1;

			if (GameServerResult.wServerType & GAME_GENRE_USERCUSTOM)
			{
				for (size_t k = 0; k  < GameServerResult.vectUserCustomTableInfo.size(); k++)
				{
					if (GameServerResult.vectUserCustomTableInfo.at(k).wTableID == (j - 1))
					{
						memcpy(m_ModuleInitParameter.GameServiceTableOption.TableOption.at(TableIndex).cbCustomRule, \
							GameServerResult.vectUserCustomTableInfo.at(k).cbCustomRule, sizeof(GameServerResult.vectUserCustomTableInfo.at(k).cbCustomRule));

						m_ModuleInitParameter.GameServiceTableOption.TableOption.at(j - 1).dwCreatUserID = GameServerResult.vectUserCustomTableInfo.at(k).dwUserID;
						m_ModuleInitParameter.GameServiceTableOption.TableOption.at(TableIndex).dwRoundCount = GameServerResult.vectUserCustomTableInfo.at(k).dwRoundCount;
						m_ModuleInitParameter.GameServiceTableOption.TableOption.at(TableIndex).dwPlayRoundCount = GameServerResult.vectUserCustomTableInfo.at(k).dwPlayRoundCount;
						m_ModuleInitParameter.GameServiceTableOption.TableOption.at(TableIndex).TableCreateTime = GameServerResult.vectUserCustomTableInfo.at(k).TableCreateTime;
						lstrcpyn(m_ModuleInitParameter.GameServiceTableOption.TableOption.at(TableIndex).szSecret, GameServerResult.vectUserCustomTableInfo.at(k).szSecret,
							sizeof(m_ModuleInitParameter.GameServiceTableOption.TableOption.at(TableIndex).szSecret));
					}
				}
			}
		}
