...
// 	// 平衡分
// protected:
// 	LONG							m_lBalanceScore;					// 用户平衡分
// 	LONG							m_lBalanceScoreBuffTime;			// 用户得到平衡分buff的时间，每局结束后进行扣除
// 	LONG							m_lBalanceScoreBuffGames;			// 用户得到平衡分buff的游戏盘数，每局结束后进行扣除
// 	LONG							m_lBalanceScoreBuffAmount;			// 用户得到平衡分buff的数量

	// 总输赢量
protected:
	SCORE							m_lTotalWin;						// 每日赢和
	SCORE							m_lTotalLose;						// 每日输和
...
public:
	...
	//用户区域ID
	virtual DWORD GetAreaID() { return m_UserInfo.dwAreaID; };
...
public:
	...
	// 设置历史记录
	virtual bool SetRecordInfo(tagVariationInfo UserRecordInfo);
	//提取变更
...
public:
	// 用户平衡积分(用来处理发牌等)
// 	virtual LONG GetUserBalanceScore();
// 	virtual VOID SetUserBalanceScore(LONG lBalanceScore);	
// 	virtual VOID SetUserBalanceScoreBuffTime(LONG dwBuffTime) { m_lBalanceScoreBuffTime = dwBuffTime;  }
// 	virtual VOID SetUserBalanceScoreBuffGames(LONG dwBuffGames) { m_lBalanceScoreBuffGames = dwBuffGames; };
// 	virtual VOID SetUserBalanceScoreBuffAmount(LONG lBuffAmount) { m_lBalanceScoreBuffAmount = lBuffAmount; };

	// 累积输赢
public:
	// 得到累积赢
	virtual SCORE GetTotalWin() { return m_lTotalWin; };
	// 得到累积输
	virtual SCORE GetTotalLose() { return m_lTotalLose; };
	// 设置累积赢
	virtual VOID SetTotalWin(SCORE lTotalWin) { m_lTotalWin = lTotalWin; };
	// 设置累积输
	virtual VOID SetTotalLose(SCORE lTotalLose) { m_lTotalLose = lTotalLose; };

	// 最大限制输赢
public:
	virtual void SetRestrictScore(SCORE lRestrictScore);
};
...
#else
typedef vector<CServerUserItem *> CServerUserItemArray;
#endif
