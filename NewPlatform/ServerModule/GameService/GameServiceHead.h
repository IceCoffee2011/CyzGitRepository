...
#include "..\..\MessageDefine\CMD_GameServer.h"//平台文件
...
#define GER_USER_TRUSTEE			0x04								//托管退出	
...
//游戏时间
#define TIME_TABLE_SINK_RANGE		30									//标识范围
...
// 默认运气值
#define DEFAULT_BALANCE_SCORE		500L			
...
struct tagRANKBONUS
{
	...
	WORD							wPropertyID[5];						//奖励道具
	BYTE							cbPropertyCount;					//道具数量
	TCHAR							szDesc[128];						//道具描述
};

//广场赛配置
struct tagGameSquareOption
{
	//启动索引
	BYTE							cbStartSquareMatch;					//启动比赛
	...
	//时间配置
	//DWORD							dwStartTickCount;					//开始时间
	//DWORD							dwEndTickCount;						//结束时间
	SYSTEMTIME						StartTime;						
	SYSTEMTIME						EndTime;						
	WORD							wInterval;							//通知间隔

	//排名与描述
	WORD							wRankNumber;						//排名限额
	TCHAR							szSquareName[1024];					//比赛名称
	TCHAR							szSquareDesc[1024];					//比赛描述
	TCHAR							szSquareBroadcast[1024];			//额外公告

	//奖励配置
	tagRANKBONUS					RankBonus[20];						//奖励细节
	BYTE							cbRankBonus;						//奖励数量
	tagRANKBONUS					RankBonusByResult[20];				//根据结果数的奖励
	BYTE							cbRankBonusByResult;				//根据结果数的奖励数量
};
...
//	tagGameServiceTableOption*		pGameServiceTableOption;			//桌子配置
	tagGameTableOption*				pTableOption;
...
//广场赛参数
struct tagSquareMatchParameter
{
	//内核组件
	ITimerEngine *					pITimerEngine;						//时间引擎
	IDataBaseEngine *				pIRecordDataBaseEngine;				//数据引擎
	...
	tagGameServiceAttrib *			pGameServiceAttrib;					//桌子属性
	tagGameServiceOption *			pGameServiceOption;					//配置参数
	tagGameSquareOption *			pGameSquareOption;					//配置广场赛
};
...
// 	LONG							lBalanceScore;						// 平衡分
// 	LONG							lBuffTime;							// Buff持续时间
// 	LONG							lBuffGames;							// Buff游戏盘数
// 	LONG							lBuffAmount;						// Buff数量
};
...
struct tagAndroidService
{
	...
	DWORD							dwSitRate;							// 机器人入座概率
	DWORD							dwLogon;							// 在具体什么分钟加入房间
	SCORE							lTakeScorePoint;					// 存款临界点
	SCORE							lSaveScorePoint;					// 取款临界点
};
...
struct tagAndroidParameter
{
	...
	DWORD							dwMinSitRate;						// 坐下的最小概率
	DWORD							dwMaxSitRate;						// 坐下的最大概率
	DWORD							dwSitRate;							// 入座概率

	SCORE							lRobotScoreMin;						// 机器人的分数小于该值时执行取款
	SCORE							lRobotScoreMax;						// 机器人取款的最大额
	int								nRobotOverflowSave;					// 机器人货币比初始货币多多少百分比时，进行存款
};
...
public:
	...
	//调整参数
	virtual bool RectifyParameter(tagGameServiceOption & GameServiceOption, tagGameTableOption& GameTableOption) = NULL;
};
...
interface IGameServiceCustomRule : public IUnknownEx
{
	...
	//销毁窗口
	virtual void DestoryCustomRule() = NULL;
};
...
public:
	...
	// 设置历史记录
	virtual bool SetRecordInfo(tagVariationInfo UserRecordInfo) = NULL;
	//提取变更
...
// public:
// 	// 用户平衡积分(用来处理发牌等)
// 	virtual LONG GetUserBalanceScore()=NULL;
// 	virtual VOID SetUserBalanceScore(LONG lBalanceScore) = NULL;		
// 	virtual VOID SetUserBalanceScoreBuffTime(LONG BuffTime) = NULL;
// 	virtual VOID SetUserBalanceScoreBuffGames(LONG BuffGames) = NULL;
// 	virtual VOID SetUserBalanceScoreBuffAmount(LONG BuffAmount) = NULL;

	// 累积输赢
public:
	// 得到累积赢
	virtual SCORE GetTotalWin() = NULL;
	// 得到累积输
	virtual SCORE GetTotalLose() = NULL;
	// 设置累积赢
	virtual VOID SetTotalWin(SCORE lTotalWin) = NULL;
	// 设置累积输
	virtual VOID SetTotalLose(SCORE lTotalLose) = NULL;

	// 最大限制输赢
public:
	virtual void SetRestrictScore(SCORE lRestrictScore) = NULL;
};
...
interface IServerUserItemSink : public IUnknownEx
{
	...
	virtual LONG CalculateBalanceScore(SCORE lOldScore, SCORE lNewScore) = NULL;
	// 获取服务配置
	virtual tagGameServiceOption* GetGameServiceOption()=NULL;
};
...
public:
	...
	virtual tagGameServiceOption * GetGameServiceOption()=NULL;
	//桌子配置
	virtual tagGameTableOption*	GetGameTableOption() = NULL;
...
public:
	...
	//系统消息
	virtual bool SendSystemMessage(LPCTSTR lpszMessage, WORD wType)=NULL;
...
public:
	...
	//旁观动作
	virtual bool PerformLookonAction(WORD wChairID, IServerUserItem * pIServerUserItem, LPCTSTR lpszPassword=NULL)=NULL;
	//坐下动作
	virtual bool PerformSitDownAction(WORD wChairID, IServerUserItem * pIServerUserItem, LPCTSTR lpszPassword=NULL)=NULL;
	// 托管退出模式下站起
	virtual bool PerformStandUpActionInTrusteeMode(IServerUserItem * pIServerUserItem) = NULL;
...
public:
	...
	// 重新初始化接口
	// 在某种模式,例如自建桌子模式中,Initialization只会被调用一次,而在桌子释放后ReInitialization会被多次调用
	virtual bool ReInitialization(IUnknownEx * pIUnknownEx) = NULL;
...
public:
	...
	//桌子结束
	virtual bool OnEventTableConclude() = 0;
	//发送场景
...
interface ITableUserAction : public IUnknownEx
{
	...
	// 用户是否允许旁观
	virtual bool OnActionUserAllowLookon(WORD wChairID, IServerUserItem * pIServerUserItem, bool bAllowLookon)=NULL;
};
...
public:
	...
	virtual LONG CalculateBalanceScore(SCORE lOldScore, SCORE lNewScore) = NULL;
	//计算广场赛积分
	virtual BYTE ResultSquareMatchScore(ITableFrame* pITableFrame, tagScoreInfo ScoreInfoArray[], WORD wScoreCount)=NULL;
	//计算广场赛积分
	virtual BYTE ResultSquareMatchScore(ITableFrame* pITableFrame, WORD wChairID, tagScoreInfo ScoreInfo)=NULL;
};
...
public:
	// 得到需要取钱的最小限额
	virtual SCORE GetTakeScorePoint() = NULL;			
	// 得到需要存钱的最大限额
	virtual SCORE GetSaveScorePoint() = NULL;
};
...
public:
	...
	//设置库存
	virtual bool SetAndroidStock(tagAndroidParameter AndroidParameter)=NULL;
	// 重置库存
	virtual bool ResetAndroidStock()=NULL;
