	...
	case SUB_CS_C_REGISTER_SERVER:	//注册房间
		{
			...
			//查找房间
			if (m_GlobalInfoManager.SearchServerItem(pRegisterServer->GameServerOption.wServerID)!=NULL)
			{
				//变量定义
				CMD_CS_S_RegisterFailure RegisterFailure;
				ZeroMemory(&RegisterFailure,sizeof(RegisterFailure));
			...
			(m_pBindParameter+wBindIndex)->wServiceID=pRegisterServer->GameServerOption.wServerID;
...
// 			lstrcpyn(GameServer.szServerAddr,pRegisterServer->szServerAddr,CountArray(GameServer.szServerAddr));
 			...
 			GameServer.wKindID=pRegisterServer->GameServerOption.wKindID;
 			GameServer.wNodeID=pRegisterServer->GameServerOption.wNodeID;
 			GameServer.wSortID=pRegisterServer->GameServerOption.wSortID;
 			GameServer.wServerID=pRegisterServer->GameServerOption.wServerID;
 			GameServer.wServerPort=pRegisterServer->GameServerOption.wServerPort;
 			GameServer.dwOnLineCount=pRegisterServer->dwOnLineCount;
 			GameServer.dwFullCount=pRegisterServer->dwFullCount;
 			lstrcpyn(GameServer.szServerName,pRegisterServer->GameServerOption.szServerName,CountArray(GameServer.szServerName));
 			lstrcpyn(GameServer.szServerAddr,pRegisterServer->szServerAddr,CountArray(GameServer.szServerAddr));
#ifndef _READ_TABLE_OPTION_
			//税收配置
			GameServer.lCellScore=pRegisterServer->GameServerOption.lCellScore;								//单位积分
			GameServer.wRevenueRatio=pRegisterServer->GameServerOption.wRevenueRatio;						//税收比例
			GameServer.lServiceScore=pRegisterServer->GameServerOption.lServiceScore;						//服务费用
			//房间配置
			GameServer.lRestrictScore=pRegisterServer->GameServerOption.lRestrictScore;						//限制积分
			GameServer.lMinTableScore=pRegisterServer->GameServerOption.lMinTableScore;						//最低积分
#else

#endif
			GameServer.lMinEnterScore=pRegisterServer->GameServerOption.lMinEnterScore;						//最低积分
			GameServer.lMaxEnterScore=pRegisterServer->GameServerOption.lMaxEnterScore;						//最高积分
			//会员限制
			GameServer.cbMinEnterMember=pRegisterServer->GameServerOption.cbMinEnterMember;					//最低会员
			GameServer.cbMaxEnterMember=pRegisterServer->GameServerOption.cbMaxEnterMember;					//最高会员
			// 房间类型
			GameServer.wServerType = pRegisterServer->GameServerOption.wServerType;
...
bool CAttemperEngineSink::OnTCPNetworkMainRemoteService(WORD wSubCmdID, VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	...
	case SUB_CS_C_USER_CUSTOM_TABLE:
		{
			// 登录服务器游→协调服务器
			// 创建房间
			//效验参数
			...
			vector<CGlobalServerItem*> vectServerItem;
			...
			do
			{
				...
				{
					// 
					if (pGlobalServerItem->m_GameServer.wKindID == pUserCustomTable->wKindID && pGlobalServerItem->m_GameServer.wServerType == GAME_GENRE_USERCUSTOM)
					{
						// 给当前服务发送信息,请求创建房间
						tagBindParameter * pBindParameter = (m_pBindParameter + pGlobalServerItem->GetIndex());
 						if (pBindParameter == nullptr)
 						{
 							continue;
 						}

						if (pBindParameter->ServiceKind == ServiceKind_Game && pBindParameter->wServiceID == pGlobalServerItem->m_GameServer.wServerID)
						{
							// 判断在线人数
							if (pGlobalServerItem->m_GameServer.dwFullCount-pGlobalServerItem->m_GameServer.dwOnLineCount <= 20)
							{
								// 放入待定的目录
								vectServerItem.push_back(pGlobalServerItem);
							}
							else
							{
								// 给对应的服务发送消息
								bSendFlag = m_pITCPNetworkEngine->SendData(pBindParameter->dwSocketID, MDM_CS_REMOTE_SERVICE, SUB_CS_S_USER_CUSTOM_TABLE, pData, wDataSize);
								if (bSendFlag)
								{
									return true;
								}
			...
			// 发送给游戏服务器失败
			CGlobalServerItem* pServerItem = nullptr;

			// 要么是没有空闲房间,要么是给房间发送失败
			if (vectServerItem.empty())
			{
				// 发送消息给协调服务器失败
				CMD_CS_C_UserCustomTableFailed UserCustomTableFailed;
				ZeroMemory(&UserCustomTableFailed, sizeof(CMD_CS_C_UserCustomTableFailed));
				UserCustomTableFailed.dwSocketID = pUserCustomTable->dwSocketID;
				lstrcpyn(UserCustomTableFailed.szDescribeString, TEXT("没有可用的游戏服务器,创建游戏房间失败!"), CountArray(UserCustomTableFailed.szDescribeString));

				//发送数据
				m_pITCPNetworkEngine->SendData(dwSocketID, MDM_CS_REMOTE_SERVICE, SUB_CS_S_USER_CUSTOM_TABLE_FAILED, &UserCustomTableFailed, sizeof(CMD_CS_C_UserCustomTableFailed));
			}
			else
			{

				for (size_t i = 0; i < vectServerItem.size(); i++)
				{
					if (pServerItem == nullptr)
					{
						pServerItem = vectServerItem.at(i);
					}
					else
					{
						if (vectServerItem.at(i)->m_GameServer.dwFullCount - vectServerItem.at(i)->m_GameServer.dwOnLineCount > pServerItem->m_GameServer.dwFullCount - pServerItem->m_GameServer.dwOnLineCount)
						{
							pServerItem = vectServerItem.at(i);
						}
					}
				...
				if (pServerItem != nullptr)
				{
					tagBindParameter * pBindParameter = (m_pBindParameter + pServerItem->GetIndex());
					if (pBindParameter == nullptr)
					{
						return true;
					}

					if (pBindParameter->ServiceKind == ServiceKind_Game && pBindParameter->wServiceID == pServerItem->m_GameServer.wServerID)
					{
						bSendFlag = m_pITCPNetworkEngine->SendData(pBindParameter->dwSocketID, MDM_CS_REMOTE_SERVICE, SUB_CS_S_USER_CUSTOM_TABLE, pData, wDataSize);
					}
				}
				...
				if (pServerItem == nullptr || bSendFlag == false)
				{
					// 发送消息给协调服务器失败
					CMD_CS_C_UserCustomTableFailed UserCustomTableFailed;
					ZeroMemory(&UserCustomTableFailed, sizeof(CMD_CS_C_UserCustomTableFailed));
					UserCustomTableFailed.dwSocketID = pUserCustomTable->dwSocketID;
					lstrcpyn(UserCustomTableFailed.szDescribeString, TEXT("没有空闲的游戏服务器,创建游戏房间失败!"), CountArray(UserCustomTableFailed.szDescribeString));
	...
	case SUB_CS_C_USER_CUSTOM_TABLE_SUCCESS:
		{
			POSITION Position = NULL;
			...
			do
			{
				...
				{
					// 给当前服务发送信息,请求创建房间
					tagBindParameter * pBindParameter = (m_pBindParameter + pGlobalPlazaItem->m_wIndex);
					if (pBindParameter == nullptr)
					{
						continue;
					}
					...
					// 给对应的服务发送消息
					m_pITCPNetworkEngine->SendData(pBindParameter->dwSocketID, MDM_CS_REMOTE_SERVICE, SUB_CS_S_USER_CUSTOM_TABLE_SUCCESS, pData, wDataSize);
				}

			} while (Position != NULL);
			return true;
		}
	case SUB_CS_C_USER_CUSTOM_TABLE_FAILED:
		{
			POSITION Position = NULL;
			//收集数据
			do
			{
				...
				if (pGlobalPlazaItem != NULL)
				{
					...
					// 给对应的服务发送消息
					m_pITCPNetworkEngine->SendData(pBindParameter->dwSocketID, MDM_CS_REMOTE_SERVICE, SUB_CS_S_USER_CUSTOM_TABLE_FAILED, pData, wDataSize);
				}
