...
BEGIN
	...
	BEGIN
		...
		@CellOnlinePresent=CellOnlinePresent,@CellOnlineTime=CellOnlineTime,@StartOnlineTime=StartOnlineTime,
		@IsPlayPresent=IsPlayPresent,@IsOnlinePresent=IsOnlinePresent
		FROM QPPlatformDB.dbo.GlobalPlayPresent WHERE ServerID=-2
		IF @ServerID IS NULL
		BEGIN
			SET @IsPlayPresent=0
	...
	BEGIN
		...
		SELECT @UserDatePresent=PresentScore FROM QPTreasureDB.dbo.StreamPlayPresent WHERE DateID=@DateID AND UserID=@dwUserID
	...
	BEGIN
		...
		SELECT @UserPresent=SUM(PresentScore) FROM QPTreasureDB.dbo.StreamPlayPresent WHERE UserID=@dwUserID
	...
	BEGIN		
		-- 更新用户金币
		UPDATE QPTreasureDB.dbo.GameScoreInfo set Score=Score+@PresentScore WHERE UserID=@dwUserID

		UPDATE QPTreasureDB.dbo.StreamPlayPresent SET PresentCount=PresentCount+1,PresentScore=PresentScore+@PresentScore,
		...
		IF @@ROWCOUNT=0
		BEGIN
			INSERT QPTreasureDB.dbo.StreamPlayPresent (DateID,UserID,PresentCount,PresentScore,PlayTimeCount,OnLineTimeCount)
