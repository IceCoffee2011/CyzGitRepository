...
BEGIN
	...
	SELECT @MedalRate=StatusValue FROM QPAccountsDB.dbo.SystemStatusInfo WHERE StatusName=N'MedalRate'

	-- 银行税率
	DECLARE @RevenueRate AS INT
	SELECT @RevenueRate=StatusValue FROM QPAccountsDB.dbo.SystemStatusInfo WHERE StatusName=N'RevenueRate'
	...
	FROM QPPlatformDB.dbo.GameGameItem TableGame,QPPlatformDB.dbo.GameKindItem TableKind
