...
BEGIN
	...
	SELECT @Nullity=Nullity, @CurrMemberOrder=MemberOrder FROM QPAccountsDB.dbo.AccountsInfo WHERE UserID=@dwSourceUserID
	...
	-- 目标用户
	IF @dwTargetUserID<>0 AND NOT EXISTS( SELECT UserID FROM QPAccountsDB.dbo.AccountsInfo WHERE UserID=@dwTargetUserID )
	BEGIN
	...
	BEGIN
		...
		SELECT @Insure=InsureScore FROM QPTreasureDB.dbo.GameScoreInfo WHERE UserID=@dwSourceUserID
		...
		-- 银行扣费
		UPDATE QPTreasureDB.dbo.GameScoreInfo SET InsureScore=InsureScore-@ConsumeGold WHERE UserID=@dwSourceUserID	
	END
	ELSE
	BEGIN
	...
	BEGIN
		...
		-- 更新会员
		UPDATE QPAccountsDB.dbo.AccountsMember SET MemberOverDate=DATEADD(mm,@MemberValidMonth, GETDATE())
		...
		FROM QPAccountsDB.dbo.AccountsMember WHERE UserID=@dwTargetUserID
	...
	-- 会员等级
	SELECT @CurrMemberOrder=MemberOrder FROM QPAccountsDB.dbo.AccountsInfo WHERE UserID=@dwTargetUserID

	-- 购买记录
	IF @wPropertyID<=13 -- 礼物
	BEGIN
		INSERT QPRecordDB.dbo.RecordSendPresent(PresentID,RcvUserID,SendUserID,LovelinessRcv,LovelinessSend,PresentPrice,PresentCount,KindID,ServerID,SendTime,ClientIP)
	...
	ELSE
	BEGIN -- 道具
		INSERT QPRecordDB.dbo.RecordSendProperty(PropID,SourceUserID,TargetUserID,PropPrice,PropCount,KindID,ServerID,SendTime,ClientIP)
