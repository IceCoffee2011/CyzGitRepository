...
{
	m_wInArea=wIssueArea;
	if (m_wInArea==PT_ISSUE_AREA_GAME)
	{
		m_btLeftMove.SetButtonImage(IDB_BT_LEFT_FRAME,GetModuleHandle(GAME_PROPERTY_DLL_NAME),true,false);
		m_btRightMove.SetButtonImage(IDB_BT_RIGHT_FRAME,GetModuleHandle(GAME_PROPERTY_DLL_NAME),true,false);
	}
	//设置变量
	m_cbPropertyCount=0;
	m_cbPresentCount=0;
...
VOID CGamePropertyCtrl::RectifyControl(INT nWidth, INT nHeight)
{
	...
	if (m_wInArea==PT_ISSUE_AREA_GAME)
	{
		nItemCount=(nWidth-40)/m_SizeImageItem.cx;
	}
...
BOOL CGamePropertyCtrl::OnEraseBkgnd(CDC * pDC)
{
	...
	if (m_wInArea==PT_ISSUE_AREA_GAME)
	{
		nItemCount=(rcClient.Width()-40)/m_SizeImageItem.cx;
	}
