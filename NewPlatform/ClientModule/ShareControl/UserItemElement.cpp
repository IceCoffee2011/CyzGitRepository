...
VOID CUserItemElement::ConstructNameCard(IClientUserItem * pIClientUserItem, IGameLevelParser * pIGameLevelParser, CImage&ImageNameCard)
{
	...
	CRect rcUserGameID(60,45,ImageSize.cx-5,45+12);
	CRect rcUserUnderWrite(60,65,ImageSize.cx-5,65+12);
	...
	if ((pCustomFaceInfo->dwDataSize!=0L)&&(pIClientUserItem->GetCustomID()!=0L))
	{
		...
		m_pIFaceItemControl->DrawFaceNormal(pDC,2,16,48,48,pCustomFaceInfo->dwCustomFace);
	}
	else
	{
		m_pIFaceItemControl->DrawFaceItemFrame(pDC,2,16,ITEM_FRAME_NORMAL);
		m_pIFaceItemControl->DrawFaceNormal(pDC,2,16,48,48,pIClientUserItem->GetFaceID());
	}
	...
	//用户标识
	_sntprintf(szString,CountArray(szString),TEXT("ID：%ld"),pIClientUserItem->GetGameID());
...
bool CUserItemElement::EfficacyPassPortID(LPCTSTR pszPassPortID, TCHAR szDescribe[], WORD wMaxCount)
{
	...
	if(nCharLength==0L) 
	{	
		lstrcpyn(szDescribe,TEXT("请输入15 位或者 18 位身份证号码！"),wMaxCount);
		return false;
	}
