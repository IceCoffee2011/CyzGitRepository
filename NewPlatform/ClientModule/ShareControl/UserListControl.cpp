...
CUserListControl::CUserListControl()
{
	...
	m_ColorUserList.crHoverTX=COLOR_HOVER_TX;
	m_ColorUserList.crHoverBK=COLOR_HOVER_BK;
	...
	case UD_GENDER:			//用户性别
		{
			...
			lstrcpyn(pszString,(cbGender==GENDER_MALE)?TEXT("男"):TEXT("女"),wMaxCount);
			break;
		}
	case UD_GROUP_NAME:		//社团名字
		{
...
VOID CUserListControl::GetItemColor(LPDRAWITEMSTRUCT lpDrawItemStruct, COLORREF&crColorText, COLORREF&crColorBack)
{
	...
	else*/ if (lpDrawItemStruct->itemID == m_dwHoverItem)
	{
		crColorText=m_ColorUserList.crHoverTX;
		crColorBack=m_ColorUserList.crHoverBK;
	}
...
VOID CUserListControl::DrawCustomItem(CDC * pDC, LPDRAWITEMSTRUCT lpDrawItemStruct, CRect&rcSubItem, INT nColumnIndex)
{
	...
	case UD_IMAGE_STATUS:	//用户状态
		{
			//索引计算
			BYTE cbGender=pIClientUserItem->GetGender()%2;
			BYTE cbUserStatus=pIClientUserItem->GetUserStatus();
			cbUserStatus = cbUserStatus==US_TRUSTEE?US_PLAYING:cbUserStatus;
