...
bool CPngImage::IsNull()
{
	...
	{
//		LOGI("CPngImage::IsNull, CD2DEngine::GetD2DEngine()");
	...
	if (m_pImage==NULL)
	{
//		LOGI("CPngImage::IsNull, m_pImage==NULL");
		return true;
	}
	if (m_pImage->GetLastStatus()!=Ok)
	{
//		LOGI("CPngImage::IsNull, m_pImage->GetLastStatus()=="<<m_pImage->GetLastStatus());
//		LOGI("CPngImage::IsNull, m_pImage->width()=="<<m_pImage->GetWidth()<<", m_pImage->height="<<m_pImage->GetHeight());
		return true;
	}
...
{
//	LOGI("CPngImage::LoadImage, pszFileName="<<pszFileName);
	//WIN 7 执行
	if ( CD2DEngine::GetD2DEngine() )
	{
#ifdef _UNICODE
		...
		Transcode::ANSI_to_Unicode(pszFileName, MAX_PATH, wstrFileName);
		CD2DEngine::GetD2DEngine()->LaodImage(NULL, wstrFileName.c_str(), m_strImageName);
#endif
	}
	...
	{
//		LOGI("m_pImage=="<<m_pImage);

//  		if (m_pImage != 0)
//  		{
// 			LOGI(_T("m_pImage->GetLastStatus()=")<<m_pImage->GetLastStatus());
//  		}
		DestroyImage();
		return false;
	}
...
{
//	LOGI("CPngImage::LoadImage, pszResourceName="<<pszResourceName);
	//WIN 7 执行
	if ( CD2DEngine::GetD2DEngine() )
	{
#ifdef _UNICODE
		...
		CD2DEngine::GetD2DEngine()->LaodImage(NULL, hInstance, wstrResourceName.c_str(), L"PNG", m_strImageName);
#endif
	}
