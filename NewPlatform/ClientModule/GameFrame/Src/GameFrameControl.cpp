...
#define USER_VIEW_CY				98							        //用户视图 223
#define MEDAL_VIEW_CY				0									//奖牌视图 36
...
VOID CGameFrameControl::RectifyControl(INT nWidth, INT nHeight)
{
	...
	INT nXPosChat=rcChatArea.left+5;
	...
	//聊天控件
	DeferWindowPos(hDwp,m_btSendChat,NULL,rcChatArea.right-45,rcChatArea.bottom-28,0,0,uFlags|SWP_NOSIZE);
	DeferWindowPos(hDwp,m_btChatShort,NULL,rcChatArea.right-73,rcChatArea.bottom-28,0,0,uFlags|SWP_NOSIZE);
	DeferWindowPos(hDwp,m_ChatEdit,NULL,rcChatArea.left+17,rcChatArea.bottom-26,rcChatArea.Width()-95,15,uFlags);
	...
	for (INT i=0;i<CountArray(ChatButtonArray);i++)
	{
		...
		DeferWindowPos(hDwp,ChatButtonArray[i]->m_hWnd,NULL,nXPosChat+ 7,nYPosChat,0,0,uFlags|SWP_NOSIZE|uTargetFlag);
...
VOID CGameFrameControl::RectifyControl(CSkinRenderManager * pSkinRenderManager)
{
	...
	m_UserListControl.SetBkColor(RGB(210,224,232));
