...
//	ASSERT(FALSE);
	...
	try
	{
		...
		IClientKernel * pIClientKernel=pGlobalUnits->m_ClientKernelModule.GetInterface();
 		if (pVideoServiceManager!=NULL&&pIClientKernel!=NULL) 
 			pVideoServiceManager->SetClientKernel(QUERY_OBJECT_PTR_INTERFACE(pIClientKernel,IUnknownEx));

		//声音引擎
		//m_D3DSound.CreateD3DSound(m_pMainWnd->GetSafeHwnd());
		m_BassSound.CreateBassSound(m_pMainWnd->GetSafeHwnd());
