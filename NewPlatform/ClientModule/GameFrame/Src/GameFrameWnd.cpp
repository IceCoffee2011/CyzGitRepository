	...
	case IDC_RULE:
		{
			try
			{
				...
				if (m_pDlgGameRule->m_hWnd==NULL)
				{
					...
					m_pDlgGameRule->Create(IDD_GAME_RULE);
...
// 		rcSplitter.right=m_nScrollXMax-EncircleInfoFrame.nRBorder;
	...
	//控制按钮
	DeferWindowPos(hDwp,m_btOption,NULL,nWidth-253,5,0,0,uFlags|SWP_NOSIZE);
	DeferWindowPos(hDwp,m_btMuteContrl,NULL,nWidth-203,5,0,0,uFlags|SWP_NOSIZE);
	DeferWindowPos(hDwp,m_btRule,NULL,nWidth-153,5,0,0,uFlags|SWP_NOSIZE);
	DeferWindowPos(hDwp,m_btMin,NULL,nWidth-100,5,0,0,uFlags|SWP_NOSIZE);
	DeferWindowPos(hDwp,m_btMax,NULL,nWidth-74,5,0,0,uFlags|SWP_NOSIZE);
	DeferWindowPos(hDwp,m_btClose,NULL,nWidth-50,5,0,0,uFlags|SWP_NOSIZE);
...
BOOL CGameFrameWnd::OnEraseBkgnd(CDC * pDC)
{
	...
	if (strTitle.IsEmpty()==false)
	{
		//计算区域
		CRect rcTitle;
		rcTitle.SetRect(35,0,rcClient.Width()-150,FrameEncircleInfo.nTBorder);
...
VOID CGameFrameWnd::OnSize(UINT nType, INT cx, INT cy) 
{
	...
	//创建区域
//	CRgn WindowRgn;
//	WindowRgn.CreateRoundRectRgn(0,0,cx+1,cy+1,ROUND_CX+3,ROUND_CY+3); //add李先生 ,框架添加圆角代码

	//设置区域
//	SetWindowRgn(WindowRgn,TRUE);

	//滚动数据(此处应该取较小值，否则会导致游戏区域被任务栏挡住)
//	m_nScrollXMax=__max(LESS_SCREEN_CX,cx);
//	m_nScrollYMax=__max(LESS_SCREEN_CY,cy);
//	m_nScrollXMax=__min(LESS_SCREEN_CX,cx);
//	m_nScrollYMax=__min(LESS_SCREEN_CY,cy);
...
INT CGameFrameWnd::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	//位置调整(此处应该取较小值，否则会导致游戏区域被任务栏挡住)
//	SizeRestrict.cx=__max(LESS_SCREEN_CX,SizeRestrict.cx);
//	SizeRestrict.cy=__max(LESS_SCREEN_CY,SizeRestrict.cy);
	SizeRestrict.cx=__min(LESS_SCREEN_CX,SizeRestrict.cx);
	SizeRestrict.cy=__min(LESS_SCREEN_CY,SizeRestrict.cy);
