...
{
	//标题属性
	m_crTitle=RGB(0,53,88);
	m_crListTX=RGB(0,53,88);
	m_crListBK=RGB(82,148,199);
...
{
	DWORD dwTempItem = HitTest(point);

	if (dwTempItem != m_dwHoverItem)
	{
		if (dwTempItem != -1)
			RedrawItems(dwTempItem,dwTempItem);
		if (m_dwHoverItem != -1)
			RedrawItems(m_dwHoverItem,m_dwHoverItem);

		m_dwHoverItem = dwTempItem;
	}
