...
CSkinButton::CSkinButton()
{
	...
	m_bRenderTextFrame=false;

	//配置变量
	m_crButtonText=RGB(0,0,0);
	m_crButtonFrameText=RGB(0,0,0);
...
//设置颜色
VOID CSkinButton::SetButtonFrameColor(COLORREF crButtonFrameText)
{
	//设置变量
	m_crButtonFrameText=crButtonFrameText;
	m_bRenderTextFrame=true;
...
VOID CSkinButton::DrawButtonText(CDC * pDC, UINT uItemState)
{
	...
	if (strText.IsEmpty()==FALSE)
	{
		...
		if(m_bRenderTextFrame)
			DrawTextString(pDC,strText,m_crButtonText,m_crButtonFrameText,rcClient);
		else
