...
void CALLBACK AudioDataCallBack(DWORD dwUserID, LPVOID lpBuf, DWORD dwLen, WAVEFORMATEX waveFormatEx, LPVOID lpUserValue)
{
	...
	if(pVideoServiceManager) 
	{
		////获取玩家
		//IClientKernel *pIClientKernel=pVideoServiceManager->GetClientKernel();
		//IClientUserItem *pUserData=pVideoServiceManager->GetClientKernel()->SearchUserByUserID(dwUserID);
		...
		////绘画音波
		//pVideoServiceControl->SetPaintVolume(true);
...
//设置音波
void CVideoServiceControl::SetVolume(DOUBLE dwCurrentVolume)
{
	//计算比例
...
//绘制音量
bool CVideoServiceControl::SetPaintVolume(bool bPaintVolume)
{
	//设置变量
	if (m_bPaintVolume!=bPaintVolume)
	{
		//设置变量
		m_bPaintVolume=bPaintVolume;
	}
...
//			CImageHandle ImageVolume(&m_ImageSmallVolume);
	...
	{
		if (m_bSmallMode==true/* && m_bPaintVolume==true*/)
		{
			//语音图片
		...
		else if(m_bSmallMode==false/* && m_bPaintVolume==true*/)
		{
			//语音图片
...
//void CVideoServiceControl::OnTimer(UINT nIDEvent) 
//{
//	switch(nIDEvent)
//	{
//	case REFRESH_SPEAKVOLUME_TIMER:	// 刷新缓冲区传输任务状态
//		{
//			m_bPaintVolume=false;
//		}
//		break;
...
VOID CVideoServiceControl::OnBnClickedVideo()
{
	...
	DWORD Return = BRAC_UserCameraControl(m_dwUserID,m_bEnableVideo);
...
bool CVideoServiceManager::SetClientKernel(IUnknownEx * pIUnknownEx)
{
	...
	if (m_hWnd==NULL)
	{
		//将VideoServiceManager窗口的父窗口设置为游戏客户端主窗口比较安全，
		//如果设置桌面窗口为父窗口，当客户端销毁的时候VideoServiceControl也随之销毁，
		//这时VideoServiceManager再对VideoServiceControl进行操作就会出现错误

		//Create(NULL,NULL,WS_CHILD,CRect(0,0,0,0),GetDesktopWindow(),300);	
		Create(NULL,NULL,WS_CHILD,CRect(0,0,0,0),AfxGetMainWnd(),300);	
...
void CVideoServiceManager::LoginVideoSystem()
{
	...
	//配置视频
	DWORD dwFuncMode = BRAC_FUNC_VIDEO_CBDATA | BRAC_FUNC_AUDIO_AUTOPLAY | BRAC_FUNC_AUDIO_CBDATA | BRAC_FUNC_AUDIO_VOLUMECALC;
	//DWORD dwFuncMode = BRAC_FUNC_VIDEO_AUTODISP | BRAC_FUNC_AUDIO_AUTOPLAY | BRAC_FUNC_AUDIO_CBDATA;
	ASSERT(m_hWnd!=NULL);
	DWORD Return = BRAC_InitSDK(m_hWnd,dwFuncMode);
	Return = BRAC_SetVideoDataCallBack(BRAC_PIX_FMT_RGB24,VideoDataCallBack,this);
	Return = BRAC_SetAudioDataCallBack(AudioDataCallBack,this);
	...
	//_snprintf(szServerIPAddr,CountArray(szServerIPAddr),"172.168.1.121");
	...
	//地址转换
	BYTE *pByte=(BYTE*)&m_dwVideoServer;
	TCHAR szServerIPAddr[30] = {0};
	_sntprintf(szServerIPAddr,CountArray(szServerIPAddr),TEXT("%d.%d.%d.%d"),pByte[0],pByte[1],pByte[2],pByte[3]);
	//_sntprintf(szServerIPAddr,CountArray(szServerIPAddr),TEXT("172.168.1.121"));

	//连接服务器
	DWORD dwReturnValue=0;
	dwReturnValue=BRAC_Connect(szServerIPAddr,m_wVideoPort);

	CString strLog;
	strLog.Format(TEXT("IP:%s,Port:%d"),szServerIPAddr,m_wVideoPort);
	//输出日志
...
WORD CVideoServiceManager::GetVideoIndex(tagUserInfo const*pagUserData)
{
	...
	//!modify by wangl 20140829 begin
	IClientUserItem *pUserItem=m_pIClientKernel->GetMeUserItem();
	for(int i = 0; i < 4; i++)
	{
		int tempseat = (pUserItem->GetChairID() + i) % 4;
		if(tempseat == pagUserData->wChairID)
			return i;
	}
	//wangl 20140829 end
	return wVideoIndex;
...
//void CVideoServiceManager::OnDestroy()
//{
//	BRAC_Release();
//	CWnd::OnDestroy();
//}
...
LRESULT CVideoServiceManager::OnGVClientConnect(WPARAM wParam, LPARAM lParam)
{
	...
	if(wParam)
	{
		...
		//!
		TCHAR szUserID[32], szVideoPassword[33];
		_sntprintf(szUserID,CountArray(szUserID),TEXT("%ld"),pUserData->dwUserID);
		CWHEncrypt::MD5Encrypt(szUserID,szVideoPassword);
		szVideoPassword[17]=0;//ID的md5前17位
		dwReturnValue=BRAC_Login(szUserID,szVideoPassword,0);

		//CHAR szUserID[32]; TCHAR szVideoPassword[33];
		//_snprintf(szUserID,CountArray(szUserID),"%ld",pUserData->dwUserID);
		//CWHEncrypt::MD5Encrypt((LPCTSTR)szUserID,szVideoPassword);
		//szVideoPassword[32]=0;//ID的md5前17位
		//dwReturnValue=BRAC_Login((LPCTSTR)szUserID,szVideoPassword,0);
...
LRESULT CVideoServiceManager::OnGVClientEnterRoom(WPARAM wParam, LPARAM lParam)
{
	...
	if (lParam==0)
	{
		...
		DWORD wReturn=BRAC_SetVideoPos(m_dwLocalUserID,pVideoServiceControl->GetSafeHwnd(),rcVideo.left,rcVideo.top,rcVideo.right,rcVideo.bottom);
		wReturn=BRAC_UserCameraControl(m_dwLocalUserID,TRUE);
		wReturn=BRAC_UserSpeakControl(m_dwLocalUserID,TRUE);
		//DWORD wReturn=BRAC_SetVideoPos(-1,pVideoServiceControl->GetSafeHwnd(),rcVideo.left,rcVideo.top,rcVideo.right,rcVideo.bottom);
		//wReturn=BRAC_UserCameraControl(-1,TRUE);
		//wReturn=BRAC_UserSpeakControl(-1,TRUE);
...
{
	//TCHAR szVideoState[128]=TEXT("");
	//_sntprintf(szVideoState, CountArray(szVideoState), TEXT("VideoState= %d"), BYTE(lParam));
	//AfxMessageBox(szVideoState);
	//玩家标识
	DWORD dwUserID=(DWORD)wParam;
	...
	DWORD dwState = -1;
	BRAC_GetCameraState(-1,dwState);
	return TRUE;
}

void CVideoServiceManager::OnTimer(UINT nIDEvent) 
{
	if(g_bClientClosed)return;
	switch(nIDEvent)
	{
	case REFRESH_SPEAKVOLUME_TIMER:	// 刷新缓冲区传输任务状态
		{
			...
			{
				CVideoServiceControl *pVideoServiceControl=m_VideoServiceControl[i];
				if(pVideoServiceControl==NULL)continue;
				DOUBLE fSpeakVolume = 0.0;
				if(m_dwUserID[i]!=-1 && BRAC_QueryUserState(m_dwUserID[i],BRAC_USERSTATE_SPEAKVOLUME,(PCHAR)&fSpeakVolume,sizeof(DOUBLE)) == GV_ERR_SUCCESS)
					pVideoServiceControl->SetVolume(fSpeakVolume);
				else
					pVideoServiceControl->SetVolume(0.f);
			}
		}
		break;
	}
