...
#define MAX_GOLD_LEVEL_COUNT           12                               //游戏币等级数
...
//获取等级
WORD CGameLevelParser::GetGameLevel(SCORE lScore)
{
	//变量定义
	WORD wLevel=0;

	INT_PTR nItemCount=(WORD)m_LevelDescribeActive.GetCount();
	...
	for (INT_PTR i=0;i<nItemCount;i++)
	{
		...
		//数据
//		LOGI("CGameLevelParser::GetGameLevel, pLevelDescribe->lLevelScore="<<pLevelDescribe->lLevelScore<<", i="<<i);
		if ((lScore>=pLevelDescribe->lLevelScore))
		{
			wLevel=i;
		}
	}

//	LOGI("CGameLevelParser::GetGameLevel, lScore="<<lScore<<", wLevel="<<wLevel<<", nItemCount="<<nItemCount);
	return (wLevel>=MAX_GOLD_LEVEL_COUNT)?MAX_GOLD_LEVEL_COUNT-1:wLevel;
}
