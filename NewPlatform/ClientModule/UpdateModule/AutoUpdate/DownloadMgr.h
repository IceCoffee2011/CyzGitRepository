...
public:
	//添加任务
	BOOL AddTask(LPCTSTR pszRemoteFile, LPCTSTR pszLocalFile=NULL, UINT nOperation=TO_DOWNLOAD|TO_INSTALL);
	//启动任务
	void Start();
	//移除任务
	void RemoveAllTask();
	...
	// 得到下载的任务数量
	INT_PTR GetTaskCount() { return m_DownloadList.GetCount(); };
