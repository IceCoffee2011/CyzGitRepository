...
BOOL CDownloadMgr::AddTask(LPCTSTR pszRemoteFile, LPCTSTR pszLocalFile, UINT nOperation /*=TO_DOWNLOAD|TO_INSTALL */)
{
	...
	pDownloadTask->nOperation = nOperation;
	_tcscpy(pDownloadTask->szRemoteFile, pszRemoteFile);
	_tcscpy(pDownloadTask->szLocalFile, (pszLocalFile ? pszLocalFile : pszRemoteFile));
...
DWORD CDownloadMgr::OnDownloadProc()
{
	...
	try
	{
		...
		for (INT_PTR i = 0; i < nCount; i++)
		{
			...
			if ( (pDownloadTask->nOperation&TO_DOWNLOAD) != TO_DOWNLOAD )
			{
			...
			pHttpFile=(CHttpFile*)m_pInteSession->OpenURL(m_strDownloadURL+pDownloadTask->szRemoteFile+EXT_ZIP, 1, 
			...
			// Create local file
			_stprintf(szLocalPath, _T("%s\\%s.dtm"), m_strTempPath, pDownloadTask->szLocalFile);

			// 获取文件夹，如果文件夹不存在，则生成文件夹
			CString strDir  = FileMisc::GetFilePath( szLocalPath );
			if( !strDir.IsEmpty() && !FileMisc::CheckFolderExist( strDir ))
			{
				if (FileMisc::CreateFolder( strDir ) == false)
				{
					throw (0);
				}
			}
...
DWORD CDownloadMgr::OnSetupFilesProc()
{
	...
	for (INT_PTR i = 0; i < nCount; i++)
	{
		...
		if ( (pDownloadTask->nOperation&TO_INSTALL) != TO_INSTALL )
		{
			continue;
		}
		...
		// 检查文件夹路径是否存在（不存在就创建文件夹 ）
		CString strDir  = FileMisc::GetFilePath( szDesFile );
		if( !strDir.IsEmpty() && !FileMisc::CheckFolderExist( strDir ))
		{
			FileMisc::CreateFolder( strDir );
		}

		// 因为是zip文件，直接解压到对应的目录，不用MoveFile来操作
		bool flag = CUnzipper::Unzip(szSrcFile, strDir, FALSE);
