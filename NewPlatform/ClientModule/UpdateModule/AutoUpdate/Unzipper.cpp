...
bool CUnzipper::UnzipTo(LPCTSTR szFolder, bool bIgnoreFilePath)
{
	...
	do
	{
		if (!UnzipFile(szFolder,NULL, bIgnoreFilePath))
			return false;
	}
	while (GotoNextFile());
...
bool CUnzipper::Unzip(LPCTSTR szFileName, LPCTSTR szFolder, bool bIgnoreFilePath)
{
	...
	bool ret = unz.UnzipTo(szFolder, bIgnoreFilePath);
	return ret;
}
...
bool CUnzipper::UnzipFile(int nFile, LPCTSTR szFolder, bool bIgnoreFilePath)
{
	...
	bool ret = UnzipFile(szFolder,NULL, bIgnoreFilePath);
...
//[Jordon 050521] 解压前的预备工作
bool CUnzipper::PerpareUnzipFile(LPTSTR lpFileName,LPCTSTR szFolder) 
{
	if (!m_uzFile)
		return FALSE;
	...
	if (NULL != lpFileName) lstrcpy(lpFileName,szFilePath);	

	if (PathFileExists(lpFileName))
		SetFileAttributes(lpFileName,FILE_ATTRIBUTE_NORMAL); //删除只读属性

	return TRUE;
}

bool CUnzipper::UnzipFile(LPCTSTR szFolder,LPTSTR lpFileName, bool bIgnoreFilePath,
						  bool bUnzipToTmpFile,LPCTSTR lpszSuffixName)
{
	if (!m_uzFile){
		return false;
	}
	...
	if (bUnzipToTmpFile)
	{
		if (lpszSuffixName == NULL)
			lstrcat(szFilePath,".tmp");
		else
			lstrcat(szFilePath,lpszSuffixName);
	}
...
bool CUnzipper::CreateFolder(LPCTSTR szFolder)
{
	...
	if (p) 
	{
		...
		*p = '\0';
		...
		// ADDED BY GEERT VAN HORRIK - START

		// Check if last character is a \n
		sTemp = szFolder;
		if (sTemp.Find('\\', sTemp.GetLength()-1) != -1)
			return true;
	...
	{
		free(szPath);	//add by jhuix in 20050521
	}
	return true;
}
...
bool CUnzipper::CloseCurrentFile()
{
	...
	int res = unzCloseCurrentFile(m_uzFile);
	if( res == UNZ_CRCERROR ) return false;
	return true;	
}

int CUnzipper::ReadCurrentFile( void* buf, unsigned len )
{
	if (!m_uzFile){
		return 0;
	}
	
	return unzReadCurrentFile(m_uzFile, buf, len);
