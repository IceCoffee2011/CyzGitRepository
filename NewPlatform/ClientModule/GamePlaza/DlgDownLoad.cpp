...
bool CDownLoadSink::PerformDownLoad(LPCTSTR pszClientName, WORD wKindID, WORD wServerID)
{
	...
	// 下载地址替换成php
//	_sntprintf(szDownLoadUrl,CountArray(szDownLoadUrl),TEXT("120.26.43.77/DownLoad.aspx?KindID=%ld"),szPlatformLink,wKindID);
//	_sntprintf(szDownLoadUrl,CountArray(szDownLoadUrl),TEXT("%s/gamedownload/download?KindID=%ld"),szPlatformLink,wKindID);
	CGlobalServer GlobalServer;
	TCHAR szGameDownloadURL[MAX_PATH]=TEXT("");
	GlobalServer.ReadFileString(_T("GameDownloadURL"), _T("GameDownloadURL"), szGameDownloadURL, CountArray(szGameDownloadURL));
//	MessageBox(szGameDownloadURL);
	_sntprintf(szDownLoadUrl, CountArray(szDownLoadUrl), szGameDownloadURL, wKindID);
