...
BOOL CDlgServicePassword::OnInitDialog()
{
	...
	// 移动控件
	m_edLogonPassword1.SetDrawBorad(false);
	m_edLogonPassword2.SetDrawBorad(false);
	m_edLogonPassword3.SetDrawBorad(false);

	m_edLogonPassword1.MoveWindow(104, 20, 180, 18);
	m_edLogonPassword2.MoveWindow(104, 62, 180, 18);
	m_edLogonPassword3.MoveWindow(104, 104, 180, 18);

	m_btLogonPassword.SetButtonImage(IDB_BT_MODIFY, hInstance, false, false);

	CRect rcLogonPassword;
	m_btLogonPassword.GetClientRect(rcLogonPassword);
	m_btLogonPassword.MoveWindow(180, 170, rcLogonPassword.Width(), rcLogonPassword.Height());
...
//	ImagePassword.LoadImage(AfxGetInstanceHandle(),TEXT("PASSWORD_LEVEL"));
	...
	CPngImage ImageServicePassword;
	ImageServicePassword.LoadImage(AfxGetInstanceHandle(),TEXT("SERVICE_PASSWORD"));
...
//	SizePassword.SetSize(ImagePassword.GetWidth()/3,ImagePassword.GetHeight()/2);
	...
	// 贴背景图片
	//绘画等级
	ImageServicePassword.DrawImage(pBufferDC,0,0,ImageServicePassword.GetWidth(),ImageServicePassword.GetHeight(),0,0);
