...
CPlazaViewItem::CPlazaViewItem() : CDialog(IDD_GAME_PLAZA)
{
	...
	m_ImageLeft.LoadFromResource(AfxGetInstanceHandle(), IDB_BROWSER_HEAD_LEFT);	
	m_ImageMiddle.LoadFromResource(AfxGetInstanceHandle(), IDB_BROWSER_HEAD_MIDDLE);	
	m_ImageRight.LoadFromResource(AfxGetInstanceHandle(), IDB_BROWSER_HEAD_RIGHT);	
...
VOID CPlazaViewItem::RectifyControl(INT nWidth, INT nHeight)
{
	...
	//	DeferWindowPos(hDwp,pSkinButtons[i]->m_hWnd,NULL,nXPos,0,0,0,uFlags|SWP_NOSIZE);
	}
	...
	DeferWindowPos(hDwp, m_PlazaBrowser.GetSafeHwnd(), NULL, 2, 28, nWidth - 3, nHeight, uFlags);
...
/*	CRect rcBrowse;
	...
	dc.FillRect(rcClient, &CBrush(RGB(199, 220, 231))); 

	m_ImageMiddle.DrawImageTile(&dc, rcClient.left + 1, rcClient.top + 2, rcClient.Width(), m_ImageMiddle.GetHeight()); 
	m_ImageLeft.Draw(dc.GetSafeHdc(), rcClient.left + 1, rcClient.top + 2);
	m_ImageRight.Draw(dc.GetSafeHdc(), rcClient.right - m_ImageRight.GetWidth() - 1, rcClient.top + 2);  

	CRect rcText = CRect(CPoint(m_ImageLeft.GetWidth() + 5, rcClient.top + 1), CSize(rcClient.Width() - m_ImageLeft.GetWidth() - 12, m_ImageMiddle.GetHeight()));
	dc.SelectObject(CSkinResourceManager::GetInstance()->GetDefaultFont());
	dc.SetBkMode(TRANSPARENT);
	dc.SetTextColor(RGB(55, 93, 110));

	TCHAR szAnnouncement[MAX_PATH] = TEXT("");
	_sntprintf(szAnnouncement, sizeof(szAnnouncement), _T("感谢您对%s的支持，我们将持续努力为大家献上最好玩的休闲棋牌游戏，祝您在游戏中快乐每一天！"), szProduct);
	dc.DrawText(szAnnouncement, rcText, DT_LEFT|DT_VCENTER|DT_END_ELLIPSIS|DT_SINGLELINE); 
