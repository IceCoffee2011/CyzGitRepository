...
BOOL CWndViewItemCtrl::OnEraseBkgnd(CDC * pDC)
{
	...
	for (INT_PTR i=0;i<m_ViewItemArray.GetCount();i++)
	{
		...
		if (pViewItemInfo->szItemTitle[0]!=0)
		{
			...
			//构造位置
			CRect rcItemText=CRect(CPoint(PointButton.x,PointButton.y+2),CSize(m_SizeButton.cx,m_SizeButton.cy));
			...
			DCBuffer.DrawText(pViewItemInfo->szItemTitle,nItemLen,&rcItemText,DT_CENTER|DT_VCENTER|DT_SINGLELINE|DT_NOCLIP);

//			DCBuffer.Rectangle(rcItemText); 
		}
	}
