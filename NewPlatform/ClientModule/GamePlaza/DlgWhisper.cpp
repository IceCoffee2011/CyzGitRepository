...
VOID CDlgWhisper::OnDrawClientArea(CDC * pDC, INT nWidth, INT nHeight)
{
	...
	rcChatInput.top=nHeight-CHAT_INPUT_CY-nBBorder-40;
	rcChatInput.bottom=nHeight-nBBorder-40;
	...
	rcChatMessage.bottom=nHeight-CHAT_INPUT_CY-nBBorder - 40;
	...
	rcChatControl.bottom=nHeight-18;
	...
	if(m_bShowHistory)
	{
		...
		rcHistory.bottom = nHeight + NORMAL_HISTORY_HEGIHT - 5 - nBBorder;
...
VOID CDlgWhisper::RectifyControl(INT nWidth, INT nHeight)
{
	...
	DeferWindowPos(hDwp,m_btSendControl,NULL,rcChatInput.right-rcSendControl.Width()-3,nHeight-rcControl.Height()-27,0,0,uFlags|SWP_NOSIZE);
	DeferWindowPos(hDwp,m_btCancel,NULL,rcChatInput.right-rcSendControl.Width()-rcControl.Width()*2-13,nHeight-rcControl.Height()-27,0,0,uFlags|SWP_NOSIZE);
	DeferWindowPos(hDwp,m_btSendChat,NULL,rcChatInput.right-rcSendControl.Width()-rcControl.Width()*1-8,nHeight-rcControl.Height()-27,0,0,uFlags|SWP_NOSIZE);
...
VOID CDlgWhisper::DrawUserInfoArea(CDC * pDC, INT nXPos, INT nYPos, INT nWidth, INT nHeight)
{
	...
	if (m_ClientUserItemArray.GetCount()>=2L)
	{
		...
		{
			pIFaceItemControl->DrawFaceNormal(pDC,nXPos+13,nYPos+14,48,48,pCustomFaceInfo->dwCustomFace);
		}
		else
		{
			pIFaceItemControl->DrawFaceNormal(pDC,nXPos+13,nYPos+14,48,48,pIClientUserItem->GetFaceID());
		}
...
*/
		//用户游戏币
		_sntprintf(szString,CountArray(szString),TEXT("银子：%I64d"),pIClientUserItem->GetUserScore());
