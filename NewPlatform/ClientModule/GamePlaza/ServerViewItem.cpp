...
//常量定义
#define USERLIST_CX					266									//用户列表
#define USERLIST_CY					282									//用户列表
#define PROPERTY_CY					96									//游戏道具
...
CServerViewItem::~CServerViewItem()
{
	...
	if (m_PenLine.GetSafeHandle() != NULL)
...
VOID CServerViewItem::DoDataExchange(CDataExchange * pDX)
{
	...
	DDX_Control(pDX, IDC_BT_TABLE_BUTTON_6, m_btTableButton6);
...
//	m_HorSplitter.SetSplitterEvent(this);
	//m_VorSplitter.SetSplitterColor(RGB(48,137,182));
	//m_HorSplitter.SetSplitterColor(RGB(48,137,182));
//	m_VorSplitter.SetSplitterColor(0.73,0.71);
//	m_HorSplitter.SetSplitterColor(0.73,0.71);
...
//	m_HorSplitter.SetSplitterType(SplitterType_Hor);
	...
	m_ChatMessage.SetExpressionManager(CExpressionManager::GetInstance(),RGB(236,244,249));
	m_ChatMessage.SetBackgroundColor(FALSE, RGB(236,244,249));

	//桌子按钮
	m_btQuitServer.SetButtonImage(IDB_BT_QUIT_SERVER,hInstance,false,false);
	m_btTableButton1.SetButtonImage(IDB_BT_TABLE_BUTTON_1,hInstance,false,false);
	m_btTableButton2.SetButtonImage(IDB_BT_TABLE_BUTTON_2,hInstance,false,false);
	m_btTableButton3.SetButtonImage(IDB_BT_TABLE_BUTTON_3,hInstance,false,false);
	m_btTableButton4.SetButtonImage(IDB_BT_TABLE_BUTTON_4,hInstance,false,false);
	m_btTableButton5.SetButtonImage(IDB_BT_TABLE_BUTTON_5,hInstance,false,false);
	m_btTableButton6.SetButtonImage(IDB_BT_TABLE_BUTTON_6,hInstance,false,false);

//	m_btTableButton1.ShowWindow(SW_HIDE);
	m_btTableButton2.ShowWindow(SW_HIDE);
	m_btTableButton3.ShowWindow(SW_HIDE);
//	m_btTableButton4.ShowWindow(SW_HIDE);
	m_btTableButton5.ShowWindow(SW_HIDE);
//	m_btTableButton6.ShowWindow(SW_HIDE);
...
//	m_btControl.SetParent(&m_VorSplitter);
  //  m_btControl.SetButtonImage((m_bHideUserList==true)?IDB_BT_USER_LIST_SHOW:IDB_BT_USER_LIST_HIDE,hInstance,true,false);
	...
	m_UserInfoDlg.Create(IDD_USER_INFO,this);
    m_UserInfoDlg.SetGameLevelParser(m_GameLevelParserModule.GetInterface());
	m_UserInfoDlg.ShowWindow(SW_HIDE);

	m_PenLine.CreatePen(PS_SOLID, 1, RGB(21,86,123)); 
	...
	case IDC_BT_TABLE_BUTTON_1:		//加入按钮
		{
			...
			//状态判断
			if (m_pIMySelfUserItem->GetUserStatus()==US_TRUSTEE)
			{
				CInformation Information(this);
				Information.ShowMessageBox(TEXT("您处于托管代打状态，操作无效，请等待当前游戏结束！"),MB_ICONINFORMATION,30L);
				return TRUE;
			}
...
//			FunctionMenu.TrackPopupMenu(rcButton.left,rcButton.bottom,this);
	...
	case IDC_BT_TABLE_BUTTON_6:		//游戏介绍
		{
			//构造地址
//			TCHAR szBrowser[MAX_PATH]=TEXT("");
//			_sntprintf(szBrowser,CountArray(szBrowser),TEXT("%s/GameRule/%ld.html"),szPlatformLink,m_GameKind.wGameID,m_GameServer.wKindID);
//			_sntprintf(szBrowser,CountArray(szBrowser),TEXT("%s/gamerule/rule?GameID=%ld&KindID=%ld"),szPlatformLink, m_GameKind.wGameID,m_GameServer.wKindID);
			...
			//构造地址
			TCHAR szBrowser[MAX_PATH]=TEXT("");
			_sntprintf(szBrowser,CountArray(szBrowser),szGameRule, m_GameKind.wGameID);
	...
	case IDM_COPY_USER_ID:
		{
			...
			IClientUserItem * pIClientUserItem=m_MenuUserItemArray[0];
			CString str;
			str.Format(TEXT("%ld"), pIClientUserItem->GetGameID());
			CWHService::SetClipboardString(str);
	...
	case IDM_LOOKON_USER:           //旁观游戏
		{
			...
			//旁观动作
			PerformLookonAction(pIClientUserItem->GetTableID(),pIClientUserItem->GetChairID(), true);
...
{
	m_UserInfoDlg.UpdateUserInfo(NULL);
	...
	if (pTableUserItem!=NULL)
	{
		...
		else
		{
			...
			//旁观动作
			PerformLookonAction(wTableID,wChairID, true);
		}
	}
	else
	{
...
VOID CServerViewItem::OnMouseSetCursor(WORD wTableID, WORD wChairID,CPoint Point)
{
    //点击位置
	if ((wTableID!=INVALID_TABLE)&&(wChairID!=INVALID_CHAIR)&&(wChairID!=INDEX_ENTER_CHAIR))
	{
		...
		/*if (pIClientUserItem!=NULL)
		{
		    m_UserInfoDlgEx.UpdateUserInfo(pIClientUserItem);

			CRect rc;
			m_UserInfoDlgEx.GetClientRect(rc);
			m_UserInfoDlgEx.MoveWindow(Point.x+50,Point.y+rc.Height()-50,rc.Width(),rc.Height());
		}
        else
        {
...
//	}
	...
	UserInfoMenu.AppendMenu(IDM_COPY_USER_ID,TEXT("复制数字账号"));
...
/*	DWORD dwPlayCount=pIClientUserItem->GetUserPlayCount();
	...
	_sntprintf(szMenuString,CountArray(szMenuString),TEXT("游戏币：") SCORE_STRING TEXT(" 总局数：%ld 逃跑率：%5.2f%%"),
		pUserInfo->lScore,dwPlayCount,fUserFleeRate);
	UserInfoMenu.AppendMenu(IDM_NULL_COMMAND,szMenuString);
...
// 	}
	...
	if ((bMeUserItem==false)&&(m_pIMySelfUserItem->GetMasterOrder()!=0))
	{
		...
		//6603玩家管理
		UINT nMenuFlags=(CMasterRight::CanLimitUserChat(m_dwMasterRight)==false)?MF_GRAYED:MF_ENABLED;
		UserInfoMenu.AppendSeparator();
		UserInfoMenu.AppendMenu(IDM_LIMIT_USER_ROOM_CHAT,TEXT("禁止该用户大厅聊天"),nMenuFlags);
		UserInfoMenu.AppendMenu(IDM_ALLOW_USER_ROOM_CHAT,TEXT("允许该用户大厅聊天"),nMenuFlags);
		UserInfoMenu.AppendMenu(IDM_LIMIT_USER_GAME_CHAT,TEXT("禁止该用户游戏聊天"),nMenuFlags);
	...
	//6603查看位置
	//6603UserInfoMenu.AppendMenu(IDM_SEE_USER_LOCATION,(bMeUserItem==true)?TEXT("查看我的位置"):TEXT("查看用户位置"));
	//6603if ((wTableID==INVALID_TABLE)||(wChairID==INVALID_CHAIR)) UserInfoMenu.EnableMenuItem(IDM_SEE_USER_LOCATION,MF_BYCOMMAND|MF_GRAYED);

	//6603功能菜单
	//6603UserInfoMenu.AppendMenu(IDM_SHOW_USER_INFO,TEXT("用户详细资料 ..."),MF_GRAYED);
	//6603UserInfoMenu.AppendMenu(IDM_USER_INFO_MANAGER,TEXT("用户信息管理 ..."),MF_GRAYED);
	UserInfoMenu.AppendMenu(IDM_SEARCH_ONLINE_USER,TEXT("查找在线用户 ..."));
	...
	case SUB_GR_CONFIG_SERVER:		//房间配置
		{
            ...
            m_UserInfoDlg.m_wServerType=m_wServerType;
			m_TableViewFrame.m_UserInfoDlgEx.m_wServerType=m_wServerType;
			...
			//获取附加信息
			TCHAR szTableNameArray[MAX_TABLE][LEN_TABLENAME];
			for(int i = 0; i < MAX_TABLE; i++) ZeroMemory(szTableNameArray[i], CountArray(szTableNameArray[i]));
			DTP_GR_TableInfo dtpTableInfo;
			memset(&dtpTableInfo, 0, sizeof(DTP_GR_TableInfo));
			//变量定义
			VOID * pDataBuffer=NULL;
			tagDataDescribe DataDescribe;
			CRecvPacketHelper RecvPacket(pConfigServer+1,wDataSize-sizeof(CMD_GR_ConfigServer));
			...
			while (true)
			{
				...
				{
				case DTP_GR_TABLE_INFO:		//桌子附加信息
					{
// 						ASSERT(pDataBuffer!=NULL);
// 						ASSERT(DataDescribe.wDataSize<=(sizeof(szTableNameArray[0])*(MAX_TABLE)));
// 						if (DataDescribe.wDataSize<=(sizeof(szTableNameArray[0])*(MAX_TABLE)))
// 						{
// 							CopyMemory(szTableNameArray,pDataBuffer,DataDescribe.wDataSize);
// 						}
						CopyMemory(&dtpTableInfo,pDataBuffer,DataDescribe.wDataSize);
						break;
					}
				}
			}
			...
			//创建桌子
			bool bSuccess=m_TableViewFrame.ConfigTableFrame(wTableCount, wChairCount, dtpTableInfo.szTableName, m_dwServerRule,m_wServerType,m_GameServer.wServerID,szResDirectory);
...
//		m_btTableButton2.EnableWindow(TRUE);
	...
	if ((m_pIMySelfUserItem!=NULL)&&(m_pIMySelfUserItem->GetTableID()!=INVALID_TABLE))
	{
		...
		{
			CInformation Information(this);
			Information.ShowMessageBox(TEXT("您处于托管代打状态，请等待当前游戏结束！"),MB_ICONINFORMATION,30L);
			return TRUE;
		}
		else
		{
...
//	_sntprintf(szBillUrl,CountArray(szBillUrl),TEXT("%s/commerical/commerical?KindID=%ld&ServerID=%ld"),szPlatformLink,m_GameKind.wKindID,m_GameServer.wServerID);
	//获取配置文件
	TCHAR szPath[MAX_PATH]=TEXT("");
	CWHService::GetWorkDirectory(szPath, sizeof(szPath));
	TCHAR szFileName[MAX_PATH]=TEXT("");
	_sntprintf(szFileName,CountArray(szFileName),TEXT("%s\\local.dat"), szPath);
	//获取地址
	TCHAR szRoomADLink[MAX_PATH]=TEXT("");
	GetPrivateProfileString(TEXT("HyperLink"),TEXT("RoomAD"),TEXT(""), szRoomADLink, sizeof(szRoomADLink), szFileName);

	//连接广告
	m_PlatformPublicize.Navigate(szRoomADLink);
...
bool CServerViewItem::OnSocketSubWisperUserChat(VOID * pData, WORD wDataSize)
{
	...
	// 判断是否是好友的消息，是好友的则自动弹出
	BYTE cbUserCompanion=pISendUserItem->GetUserCompanion();
	if (cbUserCompanion == CP_FRIEND)
	{
		CPlatformFrame* pPlatformFrame=CPlatformFrame::GetInstance();
		if (pPlatformFrame)
		{
			pPlatformFrame->ShowWhisperItem();
		}
	}
...
bool CServerViewItem::OnSocketPropertyTrumpet(VOID * pData, WORD wDataSize)
{
	...
	if (lstrlen(pSendTrumpet->szSendNickName) == 0 && lstrlen(pSendTrumpet->szTrumpetContent) == 0)
		return true;
...
bool CServerViewItem::OnSocketSubMatchFee(VOID * pData, WORD wDataSize)
{
	...
	if(dwMatchFee>0)
	{
		...
		_sntprintf(szDescribe,CountArray(szDescribe),TEXT("参赛将扣除报名费 %ld 银子，确认要参赛吗？"),dwMatchFee);
...
BOOL CServerViewItem::OnEraseBkgnd(CDC * pDC)
{
	...
	pDC->FillRect(rcClient, &CBrush(RGB(199, 219, 231)));

	//绘画桌子
	CRect rcTable;
	rcTable.top=0;
	rcTable.left=0;
	rcTable.right=rcClient.Width() - USERLIST_CX;
	rcTable.bottom=rcClient.Height();
	m_TableEncircle.DrawEncircleFrame(pDC,rcTable);

	//用户列表
	CRect rcUserList, rcChat;
	if (m_bHideUserList==false)
	{
		rcUserList.top=0;
		rcUserList.right=rcClient.Width()-3;
		rcUserList.left=rcClient.Width() - USERLIST_CX;
		rcUserList.bottom=USERLIST_CY;
	...
	if (m_bHideUserList==false)
	{
		...
		rcChat.left=rcClient.Width() - USERLIST_CX;
		rcChat.top=USERLIST_CY+PUBLICIZE_CY;
		m_ChatEncircle.DrawEncircleFrame(pDC,rcChat);
	}

	CPen* OldPen = pDC->SelectObject(&m_PenLine);
	pDC->MoveTo(rcUserList.left, rcUserList.bottom-5);
	pDC->LineTo(rcChat.left, rcChat.top);
...
//	CWndViewItemCtrl::GetInstance()->DrawLinkBoradLine(this,pDC);
	...
	//绘画标题
	CRect rcTitle(22,8,rcTable.Width()-210,20);
...
VOID CServerViewItem::RectifyControl(INT nWidth, INT nHeight)
{
	...
	//获取位置
	CRect rcVorSplitter = CRect(0, 0, 0, 0);
	CRect rcHorSplitter = CRect(0, 0, 0, 0);
...
/*	if (m_bHideUserList==false)
	{
		if (m_bRectifyVor==false)
		{
			...
			rcVorSplitter.left=nWidth-rcMainWnd.Width()*5/25;
...
/*	if ((nWidth!=0)&&(nHeight!=0))
	...
	rcTableArea.right=nWidth-USERLIST_CX;

	//列表区域
	CRect rcListArea;
	rcListArea.top=0;
	rcListArea.right=nWidth-3;
	rcListArea.left=nWidth-USERLIST_CX;
	rcListArea.bottom=USERLIST_CY;

	//聊天区域
	CRect rcChatArea;
	rcChatArea.right=nWidth;
	rcChatArea.bottom=nHeight;
	rcChatArea.left=nWidth-USERLIST_CX;
	rcChatArea.top=USERLIST_CY+PUBLICIZE_CY;
...
//	DeferWindowPos(hDwp,m_HorSplitter,NULL,rcVorSplitter.right,rcHorSplitter.top,nWidth-rcVorSplitter.right,rcHorSplitter.Height(),uFlags);
	...
	//桌子按钮
	INT nXPosTable=rcTableArea.right-77;
//	CSkinButton * TableButtonArray[]={&m_btTableButton5,&m_btTableButton4,&m_btTableButton3,&m_btTableButton2,&m_btTableButton1};
	CSkinButton * TableButtonArray[]={&m_btTableButton6, &m_btTableButton4, &m_btTableButton1};
	...
	INT nYPosChat=rcChatArea.bottom-27;
//	CSkinButton * ChatButtonArray[]={&m_btChatButton1,&m_btChatButton2,&m_btChatButton3,&m_btChatButton4};
	CSkinButton * ChatButtonArray[]={&m_btChatButton4};

	//聊天控件
	UINT uTargetFlag=(m_bHideUserList==false)?SWP_SHOWWINDOW:SWP_HIDEWINDOW;
//	DeferWindowPos(hDwp,m_btSendChat,NULL,rcChatArea.right-61,rcChatArea.bottom-27,0,0,uTargetFlag|SWP_NOSIZE);
//	DeferWindowPos(hDwp,m_btChatShort,NULL,rcChatArea.right-84,rcChatArea.bottom-25,0,0,uTargetFlag|SWP_NOSIZE);
//	DeferWindowPos(hDwp,m_ChatEdit,NULL,rcChatArea.left+10,rcChatArea.bottom-24,rcChatArea.Width()-99,14,uTargetFlag);
	...
	//移动广告控件
	DeferWindowPos(hDwp,m_PlatformPublicize,NULL,rcListArea.left+1,rcListArea.bottom,rcListArea.Width(),PUBLICIZE_CY+2,uFlags);
...
//		LOGI("rcChatControl.Width()="<<rcChatControl.Width()<<", rcChatControl.Height()="<<rcChatControl.Height());
...
VOID CServerViewItem::InitServerViewItem(CGameServerItem * pGameServerItem,WORD wAVServerPort,DWORD dwAVServerAddr)
{
	...
	//道具位置
	PropertyResource.PointExcursion.x=6;
	PropertyResource.PointExcursion.y=13;
	PropertyResource.SizeExcursion.cx=42;
	PropertyResource.SizeExcursion.cy=40;
	...
	//发起连接 
//	CString cstr;
//	cstr.Format(_T("m_GameServer.szServerAddr=%s, m_GameServer.wServerPort=%d"), m_GameServer.szServerAddr, m_GameServer.wServerPort);
//	MessageBox(cstr);
	...
	m_TableViewFrame.m_strName.Format(TEXT("%s-%s"), m_GameKind.szKindName,m_GameServer.szServerName);
//	m_TableViewFrame.m_strName = TEXT("感谢您对星际游戏中心的支持，我们将持续努力为大家献上最好玩的休闲棋牌游戏，祝您在游戏中快乐每一天！");

	//设置标题
	SetWindowText(m_TableViewFrame.m_strName);
...
VOID CServerViewItem::InitServerViewCtrl(CSkinRenderManager * pSkinRenderManager)
{
	...
	ColorUserList.crHoverTX=COLOR_HOVER_TX;
	ColorUserList.crHoverBK=COLOR_HOVER_BK;
...
//发送旁观
bool CServerViewItem::SendLookonPacket(WORD wTableID, WORD wChairID, LPCTSTR lpszPassword /*=NULL*/)
{
	//变量定义
	...
	{
		lstrcpyn(UserLookon.szPassword,lpszPassword,CountArray(UserLookon.szPassword));
	}
...
//执行旁观
bool CServerViewItem::PerformLookonAction(WORD wTableID, WORD wChairID, bool bEfficacyPass)
{
	//效验数据
	...
	//发送命令
	SendLookonPacket(wTableID,wChairID,szPassword);
...
VOID CServerViewItem::OnNMRclickUserList(NMHDR * pNMHDR, LRESULT * pResult)
{
	...
	if (pListNotify->iItem!=-1)
	{
		...
		else
		{
		    ...
		    IClientUserItem* pUserItem = ((IClientUserItem *)(lItemData));

			//CRect rcItemRect;
			//m_UserListControl.GetItemRect(pListNotify->iItem,rcItemRect,LVIR_BOUNDS);

			//CRect rcListRect;
			//m_UserListControl.GetWindowRect(rcListRect);
			//ScreenToClient(rcListRect);

			if(pUserItem==NULL)
			{
				m_TableViewFrame.m_UserInfoDlgEx.UpdateUserInfo(NULL);
			}
			else
			{
				CPoint Point;
				...
				CRect rc;
				m_TableViewFrame.m_UserInfoDlgEx.GetClientRect(rc);
				m_TableViewFrame.m_UserInfoDlgEx.UpdateUserInfo(pUserItem,m_TableViewFrame.m_strName);
				m_TableViewFrame.m_UserInfoDlgEx.SetGameLevelParser(m_TableViewFrame.m_pIGameLevelParser);
				m_TableViewFrame.m_UserInfoDlgEx.MoveWindow(Point.x, Point.y,rc.Width(),rc.Height());
				m_TableViewFrame.m_UserInfoDlgEx.ShowWindow(SW_SHOW);
			}
#else
			//6603弹出菜单
			DWORD_PTR lItemData=m_UserListControl.GetItemData(pListNotify->iItem);
...
VOID CServerViewItem::OnNMClickUserList(NMHDR * pNMHDR, LRESULT * pResult)
{
	...
	if ((pListNotify->iItem!=-1)&&(pListNotify->lParam!=NULL))
	{
		...
		{
//			ASSERT(0);
			m_UserInfoDlg.UpdateUserInfo(pIClientUserItem);

/*			CRect rcVorSplitter;
			m_VorSplitter.GetWindowRect(&rcVorSplitter);

			CRect rc;
			m_UserInfoDlg.GetClientRect(rc);
			m_UserInfoDlg.MoveWindow(rcVorSplitter.left-335,MousePoint.y,rc.Width(),rc.Height());*/
