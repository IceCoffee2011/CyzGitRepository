...
//	m_btQuit.ShowWindow(SW_HIDE);
//	m_btEnter.ShowWindow(SW_HIDE);
//	m_btOption.ShowWindow(SW_HIDE);
//	m_btRegister.ShowWindow(SW_HIDE);
...
//	m_btLogonLink1.SetButtonImage(IDB_BT_LOGON_LINK,TEXT("BT_LOGON_LINK_1"),hInstance,false,false);
	...
	//记住密码
//	m_btRem_Password.SetButtonImage(IDB_BT_LOGONG_PASS_CHK,TEXT("IDB_BT_LOGONG_PASS_CHK"),hInstance,false,false);
//	m_btRem_Password2.SetButtonImage(IDB_BT_LOGONG_PASS_UNCHK,TEXT("IDB_BT_LOGONG_PASS_UNCHK"),hInstance,false,false);
...
//  m_PlatformPublicize.SetWindowPos(NULL,LAYERED_SIZE+2.5,42,SizeWindow.cx-12-LAYERED_SIZE,140,SWP_NOZORDER|SWP_NOCOPYBITS|SWP_NOACTIVATE);
...
VOID CDlgLogon::OnSelchangeGameID()
{
	...
	//{
		...
		{
			m_btRem_Password.ShowWindow(SW_SHOW);
...
BOOL CDlgLogon::OnEraseBkgnd(CDC * pDC)
{
	...
	ImageLogo.DrawImage(pBufferDC,11,2);
	ImageTitle.DrawImage(pBufferDC,36,2);//登陆LOGO大龙
...
{
	m_cbRemPassword = m_cbRemPassword==TRUE?FALSE:TRUE;
	if (m_cbRemPassword)
	{
		m_btRem_Password.ShowWindow(SW_SHOW);
		m_btRem_Password2.ShowWindow(SW_HIDE);
