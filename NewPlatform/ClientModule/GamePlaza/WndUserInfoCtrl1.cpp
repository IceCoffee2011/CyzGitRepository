...
BOOL CWndUserInfoCtrl::OnCommand(WPARAM wParam, LPARAM lParam)
{
	...
	case IDC_USER_INFO_2:		//锁定机器
		{
			...
			_sntprintf(szNavigation,CountArray(szNavigation),TEXT("%s/Navigation%ld.aspx"),szPlatformLink,5);
	...
	case IDC_USER_INFO_4:
		{
			CPlatformFrame * pPlatformFrame=CPlatformFrame::GetInstance();
			pPlatformFrame->OnBnClickedPlatform1();
			return TRUE;
		}
	case IDC_USER_INFO_5:
		{
...
INT CWndUserInfoCtrl::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	m_btUserInfo5.Create(NULL,WS_CHILD|WS_VISIBLE|WS_DISABLED,rcCreate,this,IDC_USER_INFO_5);
	m_btUserInfo6.Create(NULL,WS_CHILD|WS_VISIBLE|WS_DISABLED,rcCreate,this,IDC_USER_INFO_6);
	m_btUserInfoSet.Create(NULL,WS_CHILD|WS_VISIBLE|WS_DISABLED,rcCreate,this,IDC_USER_INFO_SET);
	m_btUserInfoSet.ShowWindow(SW_HIDE);
	...
	m_btUserInfo4.SetButtonImage(IDB_BK_USER_INFO,TEXT("BT_USER_INFO_4"),hInstance,true,false);
	m_btUserInfo5.SetButtonImage(IDB_BK_USER_INFO,TEXT("BT_USER_INFO_5"),hInstance,true,false);
	m_btUserInfo6.SetButtonImage(IDB_BK_USER_INFO,TEXT("BT_USER_INFO_6"),hInstance,true,false);
	...
	m_ToolTipCtrl.AddTool(&m_btUserInfo1,TEXT("我的资料"));
	m_ToolTipCtrl.AddTool(&m_btUserInfo2,TEXT("推广员"));
	m_ToolTipCtrl.AddTool(&m_btUserInfo3,TEXT("魅力兑换"));
	m_ToolTipCtrl.AddTool(&m_btUserInfo4,TEXT("查找好友"));
	m_ToolTipCtrl.AddTool(&m_btUserInfo5,TEXT("消息盒"));
	m_ToolTipCtrl.AddTool(&m_btUserInfo6,TEXT("系统设置"));
...
VOID CWndUserInfoCtrl::RectifyControl(INT nWidth, INT nHeight)
{
	...
	DeferWindowPos(hDwp,m_btUserInfo1,NULL,nStartPos,nYPos,0,0,uFlags);
	DeferWindowPos(hDwp,m_btUserInfo2,NULL,nStartPos+rcButton.Width()+nSpace,nYPos,0,0,uFlags);
	DeferWindowPos(hDwp,m_btUserInfo3,NULL,nStartPos+(rcButton.Width()+nSpace)*2,nYPos,0,0,uFlags);
	DeferWindowPos(hDwp,m_btUserInfo4,NULL,nStartPos+(rcButton.Width()+nSpace)*3,nYPos,0,0,uFlags);
	DeferWindowPos(hDwp,m_btUserInfo5,NULL,nStartPos+(rcButton.Width()+nSpace)*4,nYPos,0,0,uFlags);
	DeferWindowPos(hDwp,m_btUserInfo6,NULL,nStartPos+(rcButton.Width()+nSpace)*5,nYPos,0,0,uFlags);
...
BOOL CWndUserInfoCtrl::OnEraseBkgnd(CDC * pDC)
{
	...
	if (pGlobalUserData->dwUserID!=0L)
	{
		...
		TCHAR szNameInfo[256]={0};
		_sntprintf(szNameInfo,sizeof(szNameInfo),TEXT("%s[%ld]"),pGlobalUserData->szNickName,pGlobalUserData->dwGameID);

		//输出帐号
		CRect rcNickName;
		rcNickName.SetRect(130,16,rcClient.Width()-15,28);
		pBufferDC->DrawText(szNameInfo,lstrlen(szNameInfo),&rcNickName,DT_SINGLELINE|DT_VCENTER|DT_END_ELLIPSIS);
	...
	case EVENT_USER_LOGON:			//登录成功
		{
			...
			m_btUserInfo5.EnableWindow(TRUE);
			m_btUserInfo6.EnableWindow(TRUE);
	...
	case EVENT_USER_LOGOUT:			//注销成功
		{
			...
			m_btUserInfo5.EnableWindow(FALSE);
			m_btUserInfo6.EnableWindow(FALSE);
