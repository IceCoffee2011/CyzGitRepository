...
BOOL CCollocateRoom::OnInitDialog()
{
	...
	LOGI("CCollocateRoom::OnInitDialog, width="<<rcClient.Width()<<", height="<<rcClient.Height());
	...
	//移动窗口
//	rcClient.bottom=nTViewBorder+nBFrameBorder+80;
...
void CCollocateRoom::OnPaint()
{
	CPaintDC dc(this); 

	CRect rc;
	GetDlgItem(IDC_TABLE_PASSWORD)->GetWindowRect(rc);
	ScreenToClient(rc);

	rc.DeflateRect(1, 1);

	dc.FrameRect(rc, &CBrush(RGB(131,192,219)));
}
