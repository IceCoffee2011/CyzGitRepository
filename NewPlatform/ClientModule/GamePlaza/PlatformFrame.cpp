...
#define IDC_LAST_GAME_SERVER_CTRL	304									// 最近游戏控件
...
BEGIN_MESSAGE_MAP(CPlatformFrame, CFrameWnd)
	...
	// 最近游戏控件移动
	ON_MESSAGE(WM_LASTGAMESERVERCTRL_MOVE, OnMessageLastGameServerCtrlMove)
...
bool CFrameEncircle::PreDrawEncircleImage(tagEncircleBMP & EncircleImage)
{
	...
	//构造标志
	StationLogo.DrawImage(pDC,10,28);
	EncircleImage.ImageTL.ReleaseDC();
...
CPlatformFrame::CPlatformFrame()
{
	...
	//标题窗口
	m_rcTitleText = CRect(CPoint(17, 5), CSize(400, 20));
	...
	m_ImageHeadBarLeft.LoadFromResource(AfxGetInstanceHandle(), IDB_HEAD_BAR_LEFT);		
	m_ImageHeadBarMiddle.LoadFromResource(AfxGetInstanceHandle(), IDB_HEAD_BAR_MIDDLE);	
	m_ImageHeadBarRight.LoadFromResource(AfxGetInstanceHandle(), IDB_HEAD_BAR_RIGHT);	
...
VOID CPlatformFrame::OnSplitterEvent(UINT uControlID, INT nXPos, INT nYPos)
{
	...
	if (m_bHideGameList==true)
	{
		...
		m_btControl.SetButtonImage((m_bHideGameList==true)?IDB_BT_GAME_LIST_SHOW:IDB_BT_GAME_LIST_HIDE,hInstance,true,false);
	}
...
//			DlgLockMachine.DoModal();
			...
			//构造地址
//			TCHAR szNavigation[MAX_PATH]=TEXT("");
//			_sntprintf(szNavigation,CountArray(szNavigation),TEXT("%s/pay/index.php"),szPlatformLink);
			...
			//获取地址
			TCHAR RechargeLink[256]=TEXT("");
			GetPrivateProfileString(TEXT("HyperLink"),TEXT("RechargeLink"),TEXT(""), RechargeLink, sizeof(RechargeLink), szFileName);

			//连接地址
			WebBrowse(RechargeLink,true);
	...
	case IDC_NAVIGATION+1:
		{
			// 帐号安全
			CDlgService DlgService(SERVICE_LOCK_MACHINE);
			DlgService.DoModal();
...
//			_sntprintf(szNavigation,CountArray(szNavigation),TEXT("%s/Navigation%ld.aspx"),szPlatformLink,(nCommandID-IDC_NAVIGATION)+1);
			_sntprintf(szNavigation,CountArray(szNavigation),TEXT("%s"),szPlatformLink);
	...
	case IDC_NAVIGATION+3:  // 银行
		{
			...
			//构造地址
			TCHAR szNavigation[256]=TEXT("");
			_sntprintf(szNavigation,CountArray(szNavigation),TEXT("%s/Navigation%ld.aspx"),szPlatformLink,2);
	...
	case IDC_NAVIGATION+4:		//导航按钮
		{
			...
			//构造地址
			TCHAR szNavigation[256]=TEXT("");
			_sntprintf(szNavigation,CountArray(szNavigation),TEXT("%s//zgx/getd.aspx"),szPlatformLink);
	...
	case IDC_NAVIGATION+5:			// 客服中心

// 			//构造地址
// 			TCHAR szNavigation[MAX_PATH]=TEXT("");
// 			_sntprintf(szNavigation,CountArray(szNavigation),TEXT("%s/help.html"),szPlatformLink);
			...
			_sntprintf(szFileName,CountArray(szFileName),TEXT("%s\\local.dat"), szPath);
			//获取地址
			TCHAR szCustomServiceLink[256]=TEXT("");
			GetPrivateProfileString(TEXT("HyperLink"),TEXT("CustomService"),TEXT(""), szCustomServiceLink, sizeof(szCustomServiceLink), szFileName);

			//连接地址
			WebBrowse(szCustomServiceLink,true);
	}
...
VOID CPlatformFrame::RectifyControl(INT nWidth, INT nHeight)
{
	...
	//控件位置
 	...
 	CRect rcUserInfoCtrl = CRect(CPoint(0, 0), CSize(ImageBkgnd.GetWidth(), ImageBkgnd.GetHeight()));

//	CRect rcUserInfoCtrl;
//	m_WndUserInfoCtrl.GetClientRect(rcUserInfoCtrl);
	DWORD dwGameListWidth = rcUserInfoCtrl.Width();
	if (m_bHideGameList)
	{
		dwGameListWidth = 0;
	}
	...
	//控制按钮
	DeferWindowPos(hDwp,m_btMin,NULL,nWidth-107,4,0,0,uFlags|SWP_NOSIZE);
	DeferWindowPos(hDwp,m_btMax,NULL,nWidth-76,4,0,0,uFlags|SWP_NOSIZE);
	DeferWindowPos(hDwp,m_btClose,NULL,nWidth-45,4,0,0,uFlags|SWP_NOSIZE);
	...
	for (INT i=0;i<CountArray(m_btNavigation);i++)
	{
		if (i==4)
			continue;
		DeferWindowPos(hDwp,m_btNavigation[i],NULL, 320 + btncount * 100, 26, 0, 0, uFlags|SWP_NOSIZE);
		btncount++;
	}
	...
	DeferWindowPos(hDwp,m_WndViewItemCtrl,NULL,
		EncircleInfoFrame.nLBorder+dwGameListWidth+11,EncircleInfoFrame.nTBorder+3,
		SizeItemFrame.cx,SizeItemFrame.cy,uFlags);

   	//页面位置
	m_WndViewItemCtrl.SetViewItemPostion(EncircleInfoItem.nLBorder+dwGameListWidth+1,
		EncircleInfoFrame.nTBorder+EncircleInfoItem.nTBorder,
		nWidth-EncircleInfoFrame.nLBorder-dwGameListWidth-EncircleInfoFrame.nRBorder-2,
		nHeight-EncircleInfoFrame.nTBorder-31);
    
	//其他控件
	// 玩家信息
	DeferWindowPos(hDwp,m_WndUserInfoCtrl,NULL,EncircleInfoFrame.nLBorder,EncircleInfoFrame.nTBorder - 16,dwGameListWidth,rcUserInfoCtrl.Height(),uFlags);
	// 最近游戏
	CRect rcLastGameServerCtrl;
	m_LastGameServerCtrl.GetClientRect(rcLastGameServerCtrl);
	DeferWindowPos(hDwp,m_LastGameServerCtrl,NULL,\
		EncircleInfoFrame.nLBorder+1,EncircleInfoFrame.nTBorder + rcUserInfoCtrl.Height()-16,dwGameListWidth-2, rcLastGameServerCtrl.Height(), uFlags);
	// 游戏列表
	DeferWindowPos(	hDwp,m_ServerListView,NULL,
		EncircleInfoFrame.nLBorder+1/*+EncircleInfoList.nLBorder*/,
		EncircleInfoFrame.nTBorder/*+EncircleInfoList.nTBorder+*/+rcUserInfoCtrl.Height()+rcLastGameServerCtrl.Height() - 16,
		dwGameListWidth - 2,
		nHeight-/*EncircleInfoList.nTBorder-EncircleInfoList.nBBorder	-*/EncircleInfoFrame.nTBorder-rcUserInfoCtrl.Height()-rcLastGameServerCtrl.Height()-EncircleInfoFrame.nBBorder + 16,uFlags);

	// 广告控件
	DeferWindowPos(hDwp,m_PlatformPublicize,NULL,nWidth-EncircleInfoFrame.nRBorder-210,26+(78-50)/2,200,50,uFlags);
...
bool CPlatformFrame::MaxSizeWindow()
{
	...
	if (m_bMaxShow==false)
	{
		...
		//移动窗口
		LockWindowUpdate();
		SetWindowPos(NULL,rcArce.left,rcArce.top,rcArce.Width(),rcArce.Height(),SWP_NOZORDER);
		UnlockWindowUpdate();
	}
...
// 	LOGI("CPlatformFrame::OnClose, dwUserID="<<pGlobalUserData->dwUserID );
// 	if (m_MissionLogon.GetActiveStatus() == true)
// 	{
// 		LOGI(TEXT("当前是激活状态"));
// 	}
// 	else
...
INT CPlatformFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	CRect rcLastGameServerCtrl = CRect(CPoint(0,0), CSize(ImageBkgnd.GetWidth()-2,130));
	m_LastGameServerCtrl.Create(NULL,NULL,WS_CHILD|WS_VISIBLE,rcLastGameServerCtrl,this,IDC_LAST_GAME_SERVER_CTRL);	
	...
	m_WndUserInfoCtrl.Create(NULL,NULL,WS_VISIBLE|WS_CHILD|WS_CLIPCHILDREN,rcUserInfoCtrl,this,IDC_USER_INFO_CTRL);
...
// 	m_btNavigation[4].SetButtonImage(IDB_BK_NAVIGATION,TEXT("BT_NAVIGATION_5"),hInstance,true,false);
	...
	m_btNavigation[5].SetButtonImage(IDB_BT_NAVIGATION_6,hInstance,true,false);

//	m_btNavigation[2].ShowWindow(SW_HIDE);
	m_btNavigation[4].ShowWindow(SW_HIDE);

	//拆分设置
//	m_SkinSplitter.SetSplitterEvent(this);
	m_SkinSplitter.SetSplitterColor(0.73,0.71);
	//m_SkinSplitter.SetSplitterColor(RGB(48,137,182));
	...
	m_ServerListView.SetLastGameServerSink(&m_LastGameServerCtrl);
	...
	m_ToolTipCtrl.AddTool(&m_btNavigation[0],TEXT("游戏官网"));
	m_ToolTipCtrl.AddTool(&m_btNavigation[1],TEXT("帐号安全"));
	m_ToolTipCtrl.AddTool(&m_btNavigation[2],TEXT("充值中心"));
	m_ToolTipCtrl.AddTool(&m_btNavigation[3],TEXT("财富中心"));
//	m_ToolTipCtrl.AddTool(&m_btNavigation[4],TEXT("游戏商城"))"));
	m_ToolTipCtrl.AddTool(&m_btNavigation[5],TEXT("客服中心"));
...
//	_sntprintf(szBillUrl,CountArray(szBillUrl),TEXT("%s/commerical/commerical?KindID=0&ServerID=0"),szPlatformLink);
	...
	_sntprintf(szFileName,CountArray(szFileName),TEXT("%s\\local.dat"), szPath);
	//获取地址
	TCHAR szLobbyADLink[MAX_PATH]=TEXT("");
	GetPrivateProfileString(TEXT("HyperLink"),TEXT("LobbyAD"),TEXT(""), szLobbyADLink, sizeof(szLobbyADLink), szFileName);
	m_PlatformPublicize.Navigate(szLobbyADLink);
...
// 				HINSTANCE hInstance=AfxGetInstanceHandle();
// 				m_btNavigation[2].SetButtonImage(IDB_BT_NAVIGATION_3,hInstance,true,false);
// 				m_ToolTipCtrl.AddTool(&m_btNavigation[2],TEXT("锁定本机"));
// 			}
// 			else
// 			{
// 				HINSTANCE hInstance=AfxGetInstanceHandle();
// 				m_btNavigation[2].SetButtonImage(IDB_BT_NAVIGATION_31,hInstance,true,false);
// 				m_ToolTipCtrl.AddTool(&m_btNavigation[2],TEXT("解锁本机"));
// 			}
...
// 最近游戏控件移动
LRESULT CPlatformFrame::OnMessageLastGameServerCtrlMove(WPARAM wParam, LPARAM lParam)
{
	//移动准备
	CRect rcClient;
	GetClientRect(rcClient);
	...
	m_LastGameServerCtrl.GetClientRect(rcLastGameServerCtrl);
//	DeferWindowPos(hDwp,m_LastGameServerCtrl,NULL,\
//		EncircleInfoFrame.nLBorder+1,EncircleInfoFrame.nTBorder + rcUserInfoCtrl.Height()-16,rcLastGameServerCtrl.Width(), rcLastGameServerCtrl.Height(), uFlags);
	// 游戏列表
	DeferWindowPos(	hDwp,m_ServerListView,NULL,
		EncircleInfoFrame.nLBorder+1/*+EncircleInfoList.nLBorder*/,
		EncircleInfoFrame.nTBorder/*+EncircleInfoList.nTBorder+*/+rcUserInfoCtrl.Height()+rcLastGameServerCtrl.Height() - 16,
		dwGameListWidth - 2,
		rcClient.Height()-/*EncircleInfoList.nTBorder-EncircleInfoList.nBBorder	-*/EncircleInfoFrame.nTBorder-rcUserInfoCtrl.Height()-rcLastGameServerCtrl.Height()-EncircleInfoFrame.nBBorder + 16,uFlags);
...
//#if 0
// 	dc.FillRect(rcClient, &CBrush(RGB(21, 86, 124))); 
// 
// 	//框架位置
...
/*	if (m_bHideGameList==false)
	{
		...
		//绘画界面
		dc.BitBlt(rcList.left,rcList.top,rcList.Width(),rcList.Height(),&BufferDC,0,0,SRCCOPY);
...
//	dc.FillRect(rcItem, &CBrush(RGB(255, 0, 255)));   
//#endif
	...
	CRect rcSplitter;
	m_SkinSplitter.GetWindowRect(&rcSplitter);
	BufferDC.FillRect(rcClient, &CBrush(RGB(21, 86, 124))); 
	m_FrameEncircle.DrawEncircleFrame(&BufferDC,rcClient);

	BufferDC.SetBkMode(TRANSPARENT);
	BufferDC.SelectObject(CSkinResourceManager::GetInstance()->GetDefaultFont());

	BufferDC.SetTextColor(RGB(192, 235, 255));
	CString title;

	if (m_dwAllOnLineCount == 0)
	{
		title.Format(TEXT("%s"), szProduct);
	}
	else
	{
//		title.Format(TEXT("%s - 同时在线 %ld"), szProduct, m_dwAllOnLineCount);
		// 需求去掉同时在线人数
		title.Format(TEXT("%s"), szProduct);
	}

	BufferDC.DrawText(title, m_rcTitleText, DT_VCENTER|DT_SINGLELINE|DT_LEFT);
	...
	CRect rcUserCtrl;
	m_WndUserInfoCtrl.GetWindowRect(rcUserCtrl);
	rcItem.left=FrameEncircleInfo.nLBorder+rcUserCtrl.Width();
...
//	m_ImageHeadBarMiddle.DrawImageTile(&BufferDC, rcItem.left + 268, rcItem.top, rcItem.Width(), m_ImageHeadBarRight.GetHeight()); 
//	m_ImageHeadBarLeft.Draw(BufferDC.GetSafeHdc(), rcItem.left + 268, rcItem.top);
//	m_ImageHeadBarRight.Draw(BufferDC.GetSafeHdc(), rcItem.right - m_ImageHeadBarRight.GetWidth(), rcItem.top);  */

	m_ItemEncircle.DrawEncircleFrame(&BufferDC,rcItem);
