...
bool CServerListData::InsertGameKind(tagGameKind * pGameKind)
{
	...
	if (bInsert==true)
	{
		...
		CString str;
		str.Format(TEXT("%s, %ld"), pszProcessName, pGameKindItem->m_dwProcessVersion);
//		AfxMessageBox(str);
	}
...
// 根据TypeID得到挂在当前Type节点下的类型数目
DWORD CServerListData::GetGameKindCount(WORD wTypeID)
{
	WORD wKey = 0;
	DWORD dwCount = 0;
	CGameKindItem * pGameKindItem=NULL;
	POSITION Position=m_GameKindItemMap.GetStartPosition();
	while (Position!=NULL)
	{
		m_GameKindItemMap.GetNextAssoc(Position,wKey,pGameKindItem);
		if (pGameKindItem->m_GameKind.wTypeID == wTypeID)
		{
			dwCount++;
		}
	}
