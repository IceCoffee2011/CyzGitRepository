...
public:
	...
	//配置函数
	virtual VOID InitTableView(WORD wTableID, WORD wChairCount, TCHAR szTableName[LEN_TABLENAME], ITableViewFrame * pITableFrameView)=NULL;
...
public:
	...
	//配置桌子
	virtual bool ConfigTableFrame(WORD wTableCount, WORD wChairCount, TCHAR(* szTableNameArray)[LEN_TABLENAME], DWORD dwServerRule, WORD wServerType, WORD wServerID, LPCTSTR pszResDirectory)=NULL;
...
public:
	...
	//获取接口
	virtual IGameLevelParser * GetGameLevelParser()=NULL;
	//设置接口
	virtual VOID SetGameLevelParser(IGameLevelParser * pIGameLevelParser)=NULL;
};
...
interface ITableViewFrameSink : public IUnknownEx
{
    ...
    virtual VOID OnMouseSetCursor(WORD wTableID, WORD wChairID,CPoint Point)=NULL;
	//桌子按钮
